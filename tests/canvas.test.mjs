...
test('stage client is a single canvas with pan/zoom and no leftover rail', async () => {
  ...
  const menusFile = await readFile(new URL('../src/client/stage/menus.ts', import.meta.url), 'utf8')
  assert.match(chromeFile, /智能解析/)
  ...
  assert.match(videoStudio, /时间线/)
  assert.match(videoStudio, /dx-timeline/)
  assert.match(videoStudio, /分割/)
  assert.match(videoStudio, /调色/)
  assert.match(videoStudio, /导出文件/)
  assert.match(videoStudio, /应用到预览/)
  ...
  assert.match(studioChrome, /Esc 完成 · ⌘S 保存/)
  assert.match(studioChrome, /重做/)
  assert.match(studioChrome, /完成/)
