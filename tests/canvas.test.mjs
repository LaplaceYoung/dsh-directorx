...
test('canvas store CRUD: add, connect, update, remove, arrange', async () => {
  ...
  try {
    ...
    assert.ok(second.x !== first.x || second.y !== first.y, 'unpositioned nodes must not stack on the first card')
    ...
    doc = await store.update(first.id, { path: '/tmp/vast-desolate-wilderness-in-ancient-chin-2026-08.png', label: 'vast-desolate-wilderness-in-ancient-chin-2026-08' })
    assert.equal(doc.nodes.find(node => node.id === first.id)?.label, '镜头1·改', 'file slugs must not overwrite a shot title')
...
test('canvas planBoard writes acts as groups and shots as numbered nodes', async () => {
  ...
  try {
    ...
    const secondShot = planned.doc.nodes.find(node => node.id === planned.groups[0].shotIds[1])
    assert.ok(secondShot.x > first.x, 'shots in one act sit on a horizontal strip')
...
test('canvas documents stay isolated across project roots', async () => {
  const first = await mkdtemp(join(tmpdir(), 'dx-project-a-'))
  const second = await mkdtemp(join(tmpdir(), 'dx-project-b-'))
  try {
    const store = new DirectorxCanvasStore('directorx_output')
    await runInProject(first, () => store.addNode({ kind: 'text', label: '项目A镜头' }))
    const inSecond = await runInProject(second, () => store.read())
    assert.equal(inSecond.nodes.length, 0)
    const inFirst = await runInProject(first, () => store.read())
    assert.equal(inFirst.nodes.length, 1)
    assert.equal(inFirst.nodes[0].label, '项目A镜头')
  } finally {
...
test('stage client is a single canvas with pan/zoom and no leftover rail', async () => {
  const stage = await readFile(new URL('../src/client/stage/Stage.tsx', import.meta.url), 'utf8')
  assert.match(stage, /panOnDrag/)
  assert.match(stage, /zoomOnScroll/)
  assert.match(stage, /zoomActivationKeyCode="Meta"/)
  assert.match(stage, /panActivationKeyCode="Space"/)
  assert.match(stage, /selectionOnDrag/)
  assert.match(stage, /panOnScroll/)
  assert.match(stage, /touchAction: 'none'/)
  assert.match(stage, /nodeDragThreshold=\{1\}/)
  assert.match(stage, /\/directorx\/canvas\/intent/)
  assert.match(stage, /\/directorx\/canvas\/craft/)
  assert.match(stage, /runCraft/)
  assert.match(stage, /onAskDsh/)
  assert.match(stage, /GenerateDock/)
  assert.match(stage, /\/directorx\/projects/)
  assert.match(stage, /setClientProject/)
  assert.match(stage, /ImageStudio/)
  assert.match(stage, /VideoStudio/)
  assert.match(stage, /studioTitle/)
  assert.match(stage, /openStudioFor\(focus\.id\)/)
  assert.match(stage, /StageRail/)
  assert.match(stage, /GenerateDock/)
  assert.match(stage, /AddMenu/)
  assert.match(stage, /onConnectEnd/)
  assert.match(stage, /connectHandledRef/)
  assert.match(stage, /if \(already\) return/)
  assert.match(stage, /ConnectionMode.Loose/)
  assert.match(stage, /onReconnect/)
  ...
  const chromeFile = await readFile(new URL('../src/client/stage/chrome.tsx', import.meta.url), 'utf8')
  assert.match(chromeFile, /连接到新节点/)
  assert.match(chromeFile, /剧本/)
  assert.match(chromeFile, /铺成分镜行/)
  assert.match(chromeFile, /抽帧上板/)
  assert.match(chromeFile, /按引用连线/)
  assert.match(chromeFile, /一键解析/)
  assert.match(chromeFile, /片段重做/)
  assert.match(chromeFile, /拼回成片/)
  assert.match(chromeFile, /拼成片/)
  assert.match(chromeFile, /接触表/)
  assert.match(chromeFile, /宫格切开/)
  assert.match(chromeFile, /宫格拼回/)
  assert.match(chromeFile, /分屏对照/)
  assert.match(chromeFile, /去硬字/)
  assert.match(chromeFile, /续写位/)
  assert.match(chromeFile, /导出动图/)
  assert.match(chromeFile, /改这一镜/)
  ...
  assert.match(imageStudio, /图片编辑/)
  assert.match(imageStudio, /应用裁剪/)
  assert.match(imageStudio, /重做/)
  assert.match(imageStudio, /grade-catalog/)
  assert.match(imageStudio, /GRADE_FAMILIES/)
  assert.doesNotMatch(imageStudio, /window\.prompt/)
  assert.match(imageStudio, /props.onClose\(\)/)
  assert.match(imageStudio, /exportPng\(\)/)
  assert.match(imageStudio, /setTool\('crop'\)/)
  const gradeCatalog = await readFile(new URL('../src/providers/grade-catalog.ts', import.meta.url), 'utf8')
  assert.match(gradeCatalog, /bleach-bypass/)
  assert.match(gradeCatalog, /cross-process/)
  assert.match(gradeCatalog, /open-color/)
  const videoStudio = await readFile(new URL('../src/client/stage/VideoStudio.tsx', import.meta.url), 'utf8')
  assert.match(videoStudio, /视频编辑/)
  assert.match(videoStudio, /dx-timeline/)
  assert.match(videoStudio, /在播放头分割/)
  assert.match(videoStudio, /保存到画布/)
  assert.match(videoStudio, /props.onClose\(\)/)
  assert.match(videoStudio, /exportMp4\(\)/)
  const studioChrome = await readFile(new URL('../src/client/stage/studio-chrome.tsx', import.meta.url), 'utf8')
  assert.match(studioChrome, /Esc 返回 · ⌘S 保存/)
  assert.match(stage, /SearchPalette/)
  assert.match(stage, /CompareOverlay/)
  assert.match(stage, /cycleShotStatus/)
  assert.match(stage, /SessionDock/)
  assert.match(stage, /setSessionOpen/)
  assert.match(stage, /ShortcutsSheet/)
  assert.match(stage, /toggleLock/)
  assert.match(stage, /pasteClip/)
  assert.match(stage, /pickWorkspaceSession/)
  const clientIndex = await readFile(new URL('../src/client/index.ts', import.meta.url), 'utf8')
  assert.match(clientIndex, /settings\.plugin\.item/)
  assert.match(clientIndex, /key: 'directorx'/)
  assert.match(stage, /ensureWorkspaceSession/)
  assert.match(stage, /ASK_DSH_REWRITE/)
  assert.match(stage, /startWorkspaceSession/)
  assert.match(stage, /onNewSession/)
  const sessionDock = await readFile(new URL('../src/client/stage/SessionDock.tsx', import.meta.url), 'utf8')
  assert.match(sessionDock, /\/favicon\.svg/)
  assert.match(sessionDock, /打开 DSH 会话/)
  assert.match(sessionDock, /回 DSH/)
  assert.match(sessionDock, /MarkdownView/)
  assert.match(sessionDock, /画布节点/)
  assert.match(sessionDock, /检索导演知识/)
  assert.match(sessionDock, /检索导演技能/)
  assert.match(sessionDock, /directorx_skill_route/)
  assert.match(sessionDock, /继续当前画布/)
  assert.match(sessionDock, /整理当前分镜/)
  assert.match(sessionDock, /画布工艺/)
  assert.match(sessionDock, /打开编辑台/)
  assert.match(sessionDock, /精剪当前镜头/)
  assert.match(sessionDock, /质检当前成片/)
  assert.match(sessionDock, /版权改写当前意图/)
  assert.match(sessionDock, /编排当前成片/)
  assert.match(sessionDock, /保存本次为技能/)
  assert.match(sessionDock, /保存本系列设定/)
  assert.match(sessionDock, /沿用系列包/)
  assert.match(sessionDock, /改当前镜头/)
  assert.match(sessionDock, /写场面控制表/)
  assert.match(sessionDock, /评审改编文档/)
  assert.match(sessionDock, /directorx_skill_capture/)
  assert.match(sessionDock, /directorx_bible/)
  assert.match(sessionDock, /directorx_blocking/)
  assert.match(sessionDock, /directorx_prompt_plan/)
  assert.match(sessionDock, /directorx_ip_scan/)
  assert.match(sessionDock, /directorx_edit_plan/)
  assert.match(sessionDock, /手动确认/)
  assert.match(sessionDock, /directorx_confirm/)
  assert.match(stage, /sizeFromAspect/)
  assert.match(stage, /specPrompt/)
  assert.match(stage, /incomingRefIds/)
  assert.match(stage, /nearestAspect/)
  assert.match(stage, /SelectionMode.Partial/)
  assert.match(stage, /elevateNodesOnSelect/)
  assert.match(stage, /snapToGrid/)
  assert.match(stage, /pinSubject/)
  const workstation = await readFile(new URL('../src/client/stage/NodeWorkstation.tsx', import.meta.url), 'utf8')
  assert.match(workstation, /画幅/)
  assert.match(workstation, /交给 DSH/)
  assert.match(sessionDock, /history/)
  assert.match(sessionDock, /cancel/)
  assert.match(sessionDock, /resolveLiveSession/)
  assert.match(sessionDock, /dx-tool-line/)
  assert.match(sessionDock, /dx-tool-thumb/)
  assert.match(sessionDock, /双击加入画布/)
  assert.match(sessionDock, /mediaFromToolResult/)
  assert.match(stage, /onAddMedia/)
  assert.match(sessionDock, /思考中/)
  assert.match(sessionDock, /新会话/)
  assert.match(sessionDock, /dx-ask-card/)
  assert.match(sessionDock, /QuestionCard/)
  ...
  const submitSlice = stage.slice(submitAt, submitAt + 1800)
  assert.match(submitSlice, /\/directorx\/canvas\/intent/)
  assert.doesNotMatch(submitSlice, /shotStatus: 'generating'/)
  const nodes = await readFile(new URL('../src/client/stage/nodes.tsx', import.meta.url), 'utf8')
  assert.match(nodes, /radiusCard/)
  assert.match(nodes, /onGenerate/)
  const chrome = await readFile(new URL('../src/client/stage/chrome.tsx', import.meta.url), 'utf8')
  assert.match(chrome, /reserveRight/)
  assert.match(chrome, /请输入标题/)
  assert.match(chrome, /交给 DSH 开拍/)
  assert.match(chrome, /搜索节点/)
  assert.match(chrome, /变体对比/)
  assert.match(chrome, /快捷键/)
  assert.match(chrome, /锁定/)
  assert.match(chrome, /解组/)
  assert.match(chrome, /对齐/)
  assert.match(chrome, /登记为角色/)
  assert.match(chrome, /搜索资源/)
  assert.match(chrome, /图片编辑/)
  ...
  assert.match(ipPrompt, /directorx-ask-dsh/)
  ...
  assert.match(tools, /directorx_image_edit/)
  assert.match(tools, /directorx_edit_plan/)
  assert.match(tools, /directorx_skill_route/)
  assert.match(tools, /不要用生成模型重绘来完成调色/)
  assert.match(tools, /禁止批量占位|未向用户确认/)
  assert.match(tools, /novel-characters|设定表/)
  assert.match(nodes, /keepAspectRatio/)
  ...
  assert.match(stage, /nudgeStep/)
  assert.match(chrome, /Tab 换镜/)
  assert.doesNotMatch(nodes, /NodeToolbar/)
  assert.match(chrome, /data-tip/)
  assert.match(stage, /data-tip/)
  assert.match(stage, /react-flow__minimap/)
  assert.match(stage, /bottom: 64px/)
  ...
  assert.match(stage, /onNodeDoubleClick/)
  assert.match(stage, /focusViewOptions/)
  assert.match(stage, /focusNode\(node\.id\)/)
  assert.doesNotMatch(stage, /onNodeDoubleClick[\s\S]{0,280}openStudio/)
  assert.match(nodes, /dx-card-meta/)
  assert.doesNotMatch(nodes, /dx-card-foot/)
  assert.match(nodes, /dx-media-fill/)
  assert.match(nodes, /dx-media-bleed/)
  assert.match(nodes, /displayCardTitle/)
  assert.match(nodes, /kind === 'video' && !filled/)
  assert.match(nodes, /dx-card-dock/)
  assert.match(nodes, /dx-face-fill/)
  assert.match(stage, /react-flow__resize-control \{ position: absolute/)
  assert.match(stage, /dx-media-well \{[\s\S]{0,80}position: absolute/)
  assert.match(chrome, /双击镜头居中/)
  assert.match(chrome, /编辑所选媒体/)
  assert.match(chrome, /关闭浮层 \/ 返回画布/)
...
test('details column clamp grows past the DSH 520px cap', () => {
  const floor = 360
  const chat = 320
  const clamp = (desired, viewport, sidebar) => {
    const max = Math.max(floor, viewport - sidebar - chat)
    const rounded = Math.round(desired)
    if (rounded < floor) return Math.min(floor, max)
    if (rounded > max) return max
    return rounded
  }
  assert.ok(clamp(900, 1512, 280) > 520, 'canvas column may exceed DSH DETAILS_MAX')
  assert.equal(clamp(2000, 1512, 280), 1512 - 280 - 320)
  assert.equal(clamp(100, 1512, 280), 360)
})

test('closestPorts picks facing sides instead of always right-to-left', () => {
  const left = { x: 0, y: 0, width: 200, height: 100 }
  const right = { x: 400, y: 0, width: 200, height: 100 }
  const east = closestPorts(left, right)
  assert.equal(east.sourceSide, 'right')
  assert.equal(east.targetSide, 'left')
  assert.equal(east.sourceX, 200)
  assert.equal(east.targetX, 400)
  const west = closestPorts(right, left)
  assert.equal(west.sourceSide, 'left')
  assert.equal(west.targetSide, 'right')
  const below = { x: 0, y: 300, width: 200, height: 100 }
  const south = closestPorts(left, below)
  assert.equal(south.sourceSide, 'bottom')
  assert.equal(south.targetSide, 'top')
  assert.equal(south.sourceY, 100)
  assert.equal(south.targetY, 300)
})

test('portsForHandles keep the chosen ports instead of always facing', () => {
  assert.equal(handleToSide('top'), 'top')
  assert.equal(handleToSide('in'), 'left')
  const left = { x: 0, y: 0, width: 200, height: 100 }
  const right = { x: 400, y: 20, width: 200, height: 100 }
  const forced = portsForHandles(left, right, 'bottom', 'top')
...
test('canvas intents are DSH-owned: enqueue does not write canvas nodes', async () => {
  ...
  try {
    ...
    const intent = await store.enqueue({ kind: 'video', prompt: '雨巷跟镜头', sourceId: 'src-1', selectedIds: ['src-1'], characters: ['阿宁'], model: 'MiniMax-H3', aspect: '9:16', count: 2, durationSec: 5 })
    assert.equal(intent.status, 'pending')
    assert.deepEqual(intent.characters, ['阿宁'])
    assert.equal(intent.model, 'MiniMax-H3')
    ...
    assert.match(text, /directorx_skill_route/)
    assert.match(text, /directorx_prompt_plan/)
    assert.match(text, /六要素/)
    ...
    assert.match(text, /不要用文件名覆盖镜头标题/)
    assert.match(text, /角色锚点: 阿宁/)
    assert.match(text, /模型: MiniMax-H3/)
    assert.match(text, /画幅: 9:16/)
    assert.match(text, /characters 参数/)
    const ipText = formatDshCanvasPrompt(await store.enqueue({ kind: 'video', prompt: '蜘蛛侠在楼宇间摆荡' }))
    assert.match(ipText, /directorx_ip_rewrite/)
    assert.match(ipText, /知识 213/)
    assert.doesNotMatch(ipText, /红色头罩/)
...
test('POST /directorx/canvas/intent queues a directive without mutating the board', async () => {
  ...
  try {
    ...
    assert.match(body.prompt, /不是生成提示词/)
