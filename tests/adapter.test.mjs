...
test('classify known docs as A and unknown as B', () => {
  ...
  const deepseek = classifyProviderDoc('OpenAI-compatible chat at https://api.deepseek.com/chat/completions, bearer key')
  assert.equal(deepseek.family, 'A')
  assert.equal(deepseek.mode, 'deepseek-chat')
