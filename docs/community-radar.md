...
| dnd-kit | 17,544 | MIT | 现代拖拽引擎（可访问性/传感器/碰撞） | 素材库「拖到画布」交互候选 |
...
DirectorX 画布自研视觉语言（令牌见 `src/client/canvas-theme.ts`）：

- 纯黑底、低对比点阵、Inter / 系统无衬线。
- 卡片 16px 圆角、白 6% 幽灵层、白选中描边；封面内联操作。
- 顶部中央标题输入（持久化于 canvas.json `title`）。
- 图标化幽灵方钮 + 白色胶囊主按钮；空态主按钮交给 DSH。
- **2026-08-17 UI**：去掉 zinc `#3f3f46` 灰层；令牌收在 `canvas-theme.ts`。agent 循环不在插件。
- **2026-08-17 舞台**：浮动标题 + 左侧竖栏 + 底部胶囊；缩放 5%–400%；左键拖空白平移；卡片/分组按 ghost 16px + 白描边选中。仍不写 generating 节点。
...
| Kling Agent（release-note s568cuscq1，jina 2026-08-17） | 官方页四块：①大纲/剧本→分镜剧本+元素图+分镜图+分镜视频 ②一镜扩多机位/景别且保主体 ③电商套图 ④多轮对话改图/批量出图上画布 | **skip** 第二套 loop 与电商套图；**adapt** 画布由 DSH 工具掌管：`canvas_plan`（幕/镜一次写入）/`group`/`sequence`/`node`/`disconnect`；多机位展开仍待下一波 |
...
## 人机确认 UX 调研采纳（2026-08-18）

| 模式 | 落地 |
|---|---|
| 计划→审批→执行分层 | 提案面板（画布顶栏徽标）+ proposal_next 批准优先执行环 + DSH 承接执行回填 taskId |
| 受影响对象可视化 | 悬停提案 → 画布关联节点蓝色环高亮 |
...
## 持续改进波归档（2026-08-18 中段，第 20-38 轮）

画布与编辑器累计：命令面板（19 命令）/ 底部快速添加抽屉 / 版本历史面板 / 提案审阅面板（批准执行环+快照回滚）/ agent 编辑闪烁环 / 关系高亮 / 端点圆点 / 选中计数 / 多选拖拽计数 / 粗剪条选择联动 / 标尺拖动 scrub / 播放头三态 / 闪烁检测门 / 三调色完整配方 + lut3d + 插值模式 / 字幕烧录+重叠高亮 / 状态胶囊三态 / 主题令牌化（壳层+工具卡+dock）/ 弹层动效+焦点环+微交互（组件库取值）。
验证：108/108 测试、5 路由 200、浏览器烟测（历史/适配/标题/双击抽屉）通过。


## QA 评测调研采纳（2026-08-18 收尾）

| 项 | 落地 |
|---|---|
| 冻结门 | freezedetect（≥0.5s 静止段）计数 + QA gate——冻结/黑帧/闪烁伪影三件套齐 |
| 锐度门 | sobel 边缘能量（本地校准：锐 22 / 重模糊 14，<15 判模糊） |
| 闪烁门 | 亮度差分符号交替计数（时长缩放阈值） |
