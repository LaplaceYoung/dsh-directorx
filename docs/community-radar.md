...
- **2026-08-17 UI 对齐**：去掉 zinc `#3f3f46` 灰层（与 libtv/tapnow 都不符）；令牌收在 `src/client/canvas-theme.ts`；卡片操作改封面内联；空态主按钮交给 DSH。tapnow.ai 本次 jina 仅骨架页，视觉仍以 2026-08-16 深研 + liblib.tv 公开页为准。agent 循环不在插件。
- **2026-08-17 舞台重做**：去掉顶栏/底栏堆叠；浮动标题 + 左侧竖栏 + 底部胶囊；缩放 5%–400%；左键拖空白平移；details 半屏；卡片/分组按 ghost 16px + 白描边选中。仍不写 generating 节点。
