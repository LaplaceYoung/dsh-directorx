...
> 来源：Nature「Building Blocks of Suspense」、Tandfonline「Immersive Storytelling Design Methodology」、Frontiers「PAL-D」、东京大学「沉浸认知过程」。

## 概述

**核心断言**：沉浸 = **"预期-悬念-感官的统一"**——悬念与张力源于**未来导向的预期与预测**（非叙事音乐+叙事线索增强）；PAL-D 回路指导设计；用主客观+生理信号测量迭代（衔接 293：回路应用；304：预期管理）。
...
声音/图像/环境共塑调性与沉浸——**感官统一=沉浸**（衔接 272 三通道）。

### 测量迭代（PAL-D）

主客观+生理信号（唤起度/注意）→ 多模态影响分析 → 迭代设计——**"测了再改"**（衔接 129 评测）。
...
唤起度（生理）+ 注意（眼动）+ 主观体验——**三信号**（衔接 246 眼动）。
...
沉浸设计（衔接 129/268/272/293/304）：

```text
预期 → 悬念 → 感官 → 回路 → 生成 → 测量 → 迭代
```

**DirectorX 纪律**：预期驱动悬念；感官统一；PAL-D 回路；测量迭代。

## 术语表（中英对照）

| 中文 | English | 说明 |
|---|---|---|
| 未来导向预期 | Future-Directed Expectation | 悬念来源 |
| 沉浸方法 | Immersion Methodology | 设计流程 |
| 生理信号 | Physiological Signal | 唤起度 |
| 主客观测量 | Subjective-Objective Measure | 双信号 |
| 统一氛围 | Unified Atmosphere | 感官协同 |
| 测量迭代 | Measure-Iterate | 测后改 |

## 来源

- Nature — Building Blocks of Suspense: Narrative and Music：https://www.nature.com/articles/s41599-022-01461-5
