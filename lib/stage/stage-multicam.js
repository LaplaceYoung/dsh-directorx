// ../../../src/director/optics/cuts.ts
function cameraIndex(token) {
...
function resolveCamera(cameras2, token) {
  if (token == null || cameras2.length === 0) return void 0;
  ...
  const exact = cameras2.find((item) => item.id === want);
  if (exact) return exact;
  const prefix = cameras2.filter((item) => item.id.startsWith(want) || item.label && item.label === want);
  return prefix.length === 1 ? prefix[0] : void 0;
}
function addCut(beats, tMs, cameraId, label) {
...
`;
var grid = hud.querySelector('[data-role="grid"]');
var meta = hud.querySelector('[data-role="meta"]');
...
async function preview(ids) {
  ...
  const uploaded = await fetch("/directorx/stage/api/upload", { method: "POST", body: form }).then((r) => r.json());
  return {
    ok: true,
    path: uploaded.path,
...
//# sourceMappingURL=stage-multicam.js.map
