...
(() => {
  ...
  const importAsset = async (args) => {
    ...
    if (!/^(\\/directorx\\/stage\\/files\\/uploads\\/|\\/uploads\\/|blob:|data:)/.test(url) && !/^https?:\/\/127\.0\.0\.1/.test(url)) {
  ...
  const connect = () => {
    const stream = new EventSource("/directorx/stage/api/agent/events");
    stream.onmessage = async (event) => {
      ...
      try {
        ...
        await fetch("/directorx/stage/api/agent/result", {
          method: "POST",
