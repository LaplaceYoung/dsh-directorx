...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join13(resolve10(process.cwd(), this.outputDir), CANVAS_FILE);
  }
  async read() {
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join13(resolve10(process.cwd(), this.outputDir), `canvas.json.bak-${Date.now()}`);
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join13(resolve10(process.cwd(), this.outputDir));
...
import { mkdir as mkdir10, readFile as readFile8, readdir, rm, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join14(resolve11(process.cwd(), outputDir), EDIT_SUBDIR);
...
function registerMediaListRoute(ctx, getOutputDir) {
  ...
  return webServer.register({
    ...
    handler: async (request, response) => {
      ...
      try {
        const root = resolve11(process.cwd(), getOutputDir());
        const files = [];
