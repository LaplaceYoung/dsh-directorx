...
async function videoUnderstand(input) {
  const probe = probeMedia(input.source);
  const framesDir = resolve9(process.cwd(), input.outputDir);
...
var ProposalStore = class {
  ...
  filePath() {
    return join13(resolve10(process.cwd(), this.outputDir), "proposals.json");
  }
  async read() {
    try {
...
var ProjectStyleStore = class {
  ...
  filePath() {
    return join16(resolve13(process.cwd(), this.outputDir), "style.json");
  }
  async read() {
    try {
  ...
  async set(input) {
    ...
    await mkdir12(resolve13(process.cwd(), this.outputDir), { recursive: true });
