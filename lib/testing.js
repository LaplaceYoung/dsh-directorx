...
async function pollModelverseTask(baseURL, apiKey, taskId, settings, signal, ledger) {
  ...
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve19) => setTimeout(resolve19, settings.pollIntervalMs));
...
async function minimaxH3Video(ctx, prompt, options) {
  ...
  for (let attempt = 0; attempt < ctx.settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve19) => setTimeout(resolve19, ctx.settings.pollIntervalMs));
...
import { existsSync as existsSync2, renameSync, rmSync } from "node:fs";
...
async function videoProcess(input) {
  ...
  if (input.lut3d !== void 0 && input.lut3d !== "") {
    if (!existsSync2(input.lut3d)) throw new Error(`LUT \u6587\u4EF6\u4E0D\u5B58\u5728\uFF1A${input.lut3d}`);
...
async function videoAnalyze(input) {
  ...
  const freezeResult = spawnSync4("ffmpeg", [
    "-hide_banner",
    "-i",
    input.source,
    "-vf",
  ...
  const sobelResult = spawnSync4("ffmpeg", [
    "-hide_banner",
    "-i",
    input.source,
    "-vf",
...
var DirectorxCanvasStore = class {
  ...
  snapshotsPath() {
    return join15(resolve12(process.cwd(), this.outputDir), ".canvas-snapshots");
  }
  snapshotsFile() {
    return join15(this.snapshotsPath(), "index.json");
  }
  async snapshot(label) {
    ...
    await writeFile7(join15(this.snapshotsPath(), `${id}.json`), JSON.stringify(doc, null, 2), "utf8");
    return index[0];
  }
  ...
  async restoreSnapshot(id) {
    const raw = await readFile8(join15(this.snapshotsPath(), `${id}.json`), "utf8");
    const parsed = JSON.parse(raw);
...
var CanvasIntentStore = class {
  ...
  filePath() {
    return join16(resolve13(process.cwd(), this.outputDir), FILE);
  }
  async read() {
    try {
  ...
  async write(ledger) {
    await mkdir11(resolve13(process.cwd(), this.outputDir), { recursive: true });
...
import { mkdir as mkdir12, readFile as readFile10, readdir, rm as rm2, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join17(resolve14(process.cwd(), outputDir), EDIT_SUBDIR);
...
async function listMediaFiles(outputDir) {
  const root = resolve14(process.cwd(), outputDir);
  const files = [];
...
var ProjectStyleStore = class {
  ...
  filePath() {
    return join18(resolve15(process.cwd(), this.outputDir), "style.json");
  }
  async read() {
    try {
...
var TermStore = class {
  ...
  filePath() {
    return join19(resolve16(process.cwd(), this.outputDir), "terms.json");
  }
  async read() {
    try {
  ...
  async set(entries) {
    ...
    await mkdir14(resolve16(process.cwd(), this.outputDir), { recursive: true });
...
async function contactSheet(input) {
  ...
  const out = join20(resolve17(process.cwd(), input.outputDir), `contact-sheet-${Date.now().toString(36)}.png`);
...
// src/cases/run.ts
import { writeFile as writeFile11, mkdir as mkdir15 } from "node:fs/promises";
...
async function runProductionCase(input) {
  ...
  const stages = [];
  const tools = (name, args, output) => ({ name, input: args, output });
  ...
  const run = {
    ...
    reportPath: join21(resolve18(process.cwd(), input.outputDir), `case-${id}.json`)
  };
  await mkdir15(resolve18(process.cwd(), input.outputDir), { recursive: true });
