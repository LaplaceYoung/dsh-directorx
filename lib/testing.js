...
// src/tasks.ts
import { appendFile as appendFile2, mkdir as mkdir3, readFile as readFile4 } from "node:fs/promises";
...
`, "utf8");
  ...
  async list() {
    const path = await this.filePath();
    const content = await readFile4(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
    const records = [];
...
async function mockAudio(ctx, text) {
  ...
  const path = join6(outDir, `${slugify(text, 24)}-mock.wav`);
...
async function openaiTts(ctx, text, voice, format, instructions, speed) {
  ...
  const path = join6(outDir, `${slugify(text, 24)}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}.${ext}`);
...
async function pollModelverseTask(baseURL, apiKey, taskId, settings, signal, ledger) {
  ...
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve11) => setTimeout(resolve11, settings.pollIntervalMs));
...
async function minimaxH3Video(ctx, prompt, options) {
  ...
  for (let attempt = 0; attempt < ctx.settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve11) => setTimeout(resolve11, ctx.settings.pollIntervalMs));
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join8(outDir, `${slugify(prompt)}-mock.mp4`);
...
async function extractFrames(source, outputDir, options = {}) {
  ...
  const dir = join9(resolveOutputDir(outputDir), "frames");
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join10(resolveOutputDir(ctx.settings.outputDir), "transcripts");
  await mkdir5(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join10(dir, `${slugify(source, 24)}-${stamp}.srt`);
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile5(resolve5(source));
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join10(dir, `${slugify(source, 24)}-${stamp}.txt`);
...
var CharacterStore = class {
  ...
  filePath() {
    return join13(resolveOutputDir(this.outputDir), "characters.json");
  }
  async read() {
    try {
...
function segmentCachePath(outputDir, fingerprint) {
  return join14(resolveOutputDir(outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join14(resolveOutputDir(outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join14(resolveOutputDir(outputDir), `faded-${Date.now().toString(36)}.mp4`);
...
var ProposalStore = class {
  ...
  filePath() {
    return join15(resolveOutputDir(this.outputDir), "proposals.json");
  }
  async read() {
    try {
...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join16(resolveOutputDir(this.outputDir), CANVAS_FILE);
  }
  async read() {
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join16(resolveOutputDir(this.outputDir), `canvas.json.bak-${Date.now()}`);
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join16(resolveOutputDir(this.outputDir));
  ...
  snapshotsPath() {
    return join16(resolveOutputDir(this.outputDir), ".canvas-snapshots");
  }
  snapshotsFile() {
    return join16(this.snapshotsPath(), "index.json");
  }
  async snapshot(label) {
    ...
    await mkdir10(this.snapshotsPath(), { recursive: true });
    await writeFile7(this.snapshotsFile(), JSON.stringify(index, null, 2), "utf8");
    await writeFile7(join16(this.snapshotsPath(), `${id}.json`), JSON.stringify(doc, null, 2), "utf8");
    return index[0];
  }
  ...
  async restoreSnapshot(id) {
    const raw = await readFile8(join16(this.snapshotsPath(), `${id}.json`), "utf8");
    const parsed = JSON.parse(raw);
...
import { mkdir as mkdir11, readFile as readFile9, writeFile as writeFile8 } from "node:fs/promises";
...
var CanvasIntentStore = class {
  ...
  filePath() {
    return join17(resolveOutputDir(this.outputDir), FILE);
  }
  async read() {
    try {
...
import { mkdir as mkdir12, readFile as readFile10, writeFile as writeFile9 } from "node:fs/promises";
...
var StudioTicketStore = class {
  ...
  filePath() {
    return join18(resolveOutputDir(this.outputDir), FILE2);
  }
  async read() {
    try {
  ...
  async write(input) {
    ...
    await writeFile9(this.filePath(), JSON.stringify(ticket), "utf8");
    return ticket;
  }
};
...
import { mkdir as mkdir13, readFile as readFile11, readdir, rm as rm2, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join19(resolveOutputDir(outputDir), EDIT_SUBDIR);
...
import { mkdir as mkdir14, readFile as readFile12, writeFile as writeFile10 } from "node:fs/promises";
...
var ProjectStyleStore = class {
  ...
  filePath() {
    return join20(resolveOutputDir(this.outputDir), "style.json");
  }
  async read() {
    try {
  ...
  async set(input) {
    ...
    await writeFile10(this.filePath(), JSON.stringify(merged, null, 2), "utf8");
    return merged;
  }
  /** 生成提示词的常量块（逐字复用）。 */
  block() {
...
// src/terms.ts
import { mkdir as mkdir15, readFile as readFile13, writeFile as writeFile11 } from "node:fs/promises";
...
var TermStore = class {
  ...
  filePath() {
    return join21(resolveOutputDir(this.outputDir), "terms.json");
  }
  async read() {
    try {
  ...
  async set(entries) {
    ...
    await writeFile11(this.filePath(), JSON.stringify({ terms: ledger }, null, 2), "utf8");
    return ledger;
  }
  /** 按句命中：返回文本中出现的术语及其读法。 */
  async match(text) {
...
async function contactSheet(input) {
  ...
  const out = join22(resolveOutputDir(input.outputDir), `contact-sheet-${Date.now().toString(36)}.png`);
...
import { mkdir as mkdir16, writeFile as writeFile12 } from "node:fs/promises";
...
async function orchestrateProduction(input) {
  ...
  const run = {
    ...
    reportPath: join23(resolveOutputDir(input.outputDir), `orchestrate-${kind}.json`)
  };
  await mkdir16(resolveOutputDir(input.outputDir), { recursive: true });
  await writeFile12(run.reportPath, JSON.stringify(run, null, 2), "utf8");
  return run;
}
