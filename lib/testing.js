...
var DirectorxCanvasStore = class {
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join10(resolve7(process.cwd(), this.outputDir), `canvas.json.bak-${Date.now()}`);
...
import { mkdir as mkdir7, readFile as readFile7, readdir, rm, stat } from "node:fs/promises";
