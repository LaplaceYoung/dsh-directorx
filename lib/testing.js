...
var ProductionStageStore = class {
  ...
  filePath() {
    return join4(resolveOutputDir(this.outputDir), "stage.json");
  }
  async read() {
    try {
  ...
  async write(doc) {
    ...
    await writeFile2(this.filePath(), JSON.stringify(doc, null, 2), "utf8");
    return doc;
  }
  async get() {
...
// src/tasks.ts
import { appendFile as appendFile2, mkdir as mkdir4, readFile as readFile6 } from "node:fs/promises";
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join10(outDir, `${slugify(prompt)}-mock.mp4`);
...
async function extractFrames(source, outputDir, options = {}) {
  ...
  const dir = join11(resolveOutputDir(outputDir), "frames");
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join12(resolveOutputDir(ctx.settings.outputDir), "transcripts");
  await mkdir6(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join12(dir, `${slugify(source, 24)}-${stamp}.srt`);
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile7(resolve6(source));
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join12(dir, `${slugify(source, 24)}-${stamp}.txt`);
...
import { extname as extname2, join as join13 } from "node:path";
...
async function applyGrade(input) {
  ...
  const out = join13(resolveOutputDir(input.outputDir), `${slugify(`grade-${input.look}`)}-${Date.now().toString(36)}${ext}`);
...
function outputPath(outputDir, tag, ext) {
  ...
  mkdir7(root, { recursive: true }).catch(() => {
  });
  return join14(root, `${slugify(tag)}-${Date.now().toString(36)}.${ext}`);
}
...
// src/characters.ts
import { mkdir as mkdir8, readFile as readFile8, writeFile as writeFile6 } from "node:fs/promises";
...
var CharacterStore = class {
  ...
  filePath() {
    return join15(resolveOutputDir(this.outputDir), "characters.json");
  }
  async read() {
    try {
...
function segmentCachePath(outputDir, fingerprint) {
  return join16(resolveOutputDir(outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join16(resolveOutputDir(outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join16(resolveOutputDir(outputDir), `faded-${Date.now().toString(36)}.mp4`);
...
async function videoUnderstand(input) {
  ...
  await mkdir9(framesDir, { recursive: true });
...
// src/proposals.ts
import { mkdir as mkdir10, readFile as readFile9, writeFile as writeFile7 } from "node:fs/promises";
...
var ProposalStore = class {
  ...
  filePath() {
    return join17(resolveOutputDir(this.outputDir), "proposals.json");
  }
  async read() {
    try {
  ...
  async write(ledger) {
    ...
    await writeFile7(this.filePath(), JSON.stringify(ledger, null, 2), "utf8");
    return ledger;
  }
...
// src/canvas.ts
import { mkdir as mkdir11, readFile as readFile10, writeFile as writeFile8 } from "node:fs/promises";
...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join18(resolveOutputDir(this.outputDir), CANVAS_FILE);
  }
  async read() {
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join18(resolveOutputDir(this.outputDir), `canvas.json.bak-${Date.now()}`);
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join18(resolveOutputDir(this.outputDir));
  ...
  snapshotsPath() {
    return join18(resolveOutputDir(this.outputDir), ".canvas-snapshots");
  }
  snapshotsFile() {
    return join18(this.snapshotsPath(), "index.json");
  }
  async snapshot(label) {
    ...
    await mkdir11(this.snapshotsPath(), { recursive: true });
    await writeFile8(this.snapshotsFile(), JSON.stringify(index, null, 2), "utf8");
    await writeFile8(join18(this.snapshotsPath(), `${id}.json`), JSON.stringify(doc, null, 2), "utf8");
    return index[0];
  }
  ...
  async restoreSnapshot(id) {
    const raw = await readFile10(join18(this.snapshotsPath(), `${id}.json`), "utf8");
    const parsed = JSON.parse(raw);
...
var CanvasIntentStore = class {
  ...
  filePath() {
    return join19(resolveOutputDir(this.outputDir), FILE);
  }
  async read() {
    try {
...
var StudioTicketStore = class {
  ...
  filePath() {
    return join20(resolveOutputDir(this.outputDir), FILE2);
  }
  async read() {
    try {
  ...
  async write(input) {
    ...
    await writeFile10(this.filePath(), JSON.stringify(ticket), "utf8");
    return ticket;
  }
};
...
import { mkdir as mkdir14, readFile as readFile13, readdir as readdir2, rm as rm2, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join21(resolveOutputDir(outputDir), EDIT_SUBDIR);
...
// src/terms.ts
import { mkdir as mkdir16, readFile as readFile15, writeFile as writeFile12 } from "node:fs/promises";
...
async function contactSheet(input) {
  ...
  const out = join24(resolveOutputDir(input.outputDir), `contact-sheet-${Date.now().toString(36)}.png`);
...
var AdapterStore = class {
  ...
  catalogPath() {
    return join25(resolveOutputDir(this.outputDir), "adapters.json");
  }
  secretsPath() {
    return join25(resolveOutputDir(this.outputDir), "adapters.secrets.json");
  }
  docPath(id) {
    return join25(resolveOutputDir(this.outputDir), "adapters", `${id}.doc.txt`);
  }
  async read() {
    try {
      const parsed = JSON.parse(await readFile16(this.catalogPath(), "utf8"));
  ...
  async readDoc(id) {
    try {
      return await readFile16(this.docPath(id), "utf8");
    } catch {
      return "";
    }
  }
  async readSecrets() {
    try {
      const parsed = JSON.parse(await readFile16(this.secretsPath(), "utf8"));
  ...
  async writeSecret(id, secret) {
    ...
    await writeFile13(this.secretsPath(), JSON.stringify(all), "utf8");
  }
  async hasSecret(id) {
  ...
  async writeCatalog(file) {
    await mkdir17(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile13(this.catalogPath(), JSON.stringify(file, null, 2), "utf8");
  }
  async upsert(record) {
  ...
  async saveDoc(id, text) {
    ...
    await mkdir17(join25(resolveOutputDir(this.outputDir), "adapters"), { recursive: true });
    await writeFile13(this.docPath(id), clipped, "utf8");
...
async function orchestrateProduction(input) {
  ...
  const run = {
    ...
    reportPath: join26(resolveOutputDir(input.outputDir), `orchestrate-${kind}.json`)
  };
  await mkdir18(resolveOutputDir(input.outputDir), { recursive: true });
  await writeFile14(run.reportPath, JSON.stringify(run, null, 2), "utf8");
  return run;
}
