...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join11(resolve8(process.cwd(), this.outputDir), CANVAS_FILE);
  }
  async read() {
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join11(resolve8(process.cwd(), this.outputDir), `canvas.json.bak-${Date.now()}`);
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join11(resolve8(process.cwd(), this.outputDir));
...
import { mkdir as mkdir8, readFile as readFile7, readdir, rm, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join12(resolve9(process.cwd(), outputDir), EDIT_SUBDIR);
...
function registerMediaListRoute(ctx, getOutputDir) {
  ...
  return webServer.register({
    ...
    handler: async (request, response) => {
      ...
      try {
        const root = resolve9(process.cwd(), getOutputDir());
        const files = [];
