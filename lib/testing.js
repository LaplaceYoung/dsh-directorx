...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join8(resolveOutputDir(this.outputDir), CANVAS_FILE);
  }
  async read() {
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join8(resolveOutputDir(this.outputDir), `canvas.json.bak-${Date.now()}`);
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join8(resolveOutputDir(this.outputDir));
  ...
  snapshotsPath() {
    return join8(resolveOutputDir(this.outputDir), ".canvas-snapshots");
  }
  snapshotsFile() {
    return join8(this.snapshotsPath(), "index.json");
  }
  async snapshot(label) {
    ...
    await mkdir5(this.snapshotsPath(), { recursive: true });
    await writeFile5(this.snapshotsFile(), JSON.stringify(index, null, 2), "utf8");
    await writeFile5(join8(this.snapshotsPath(), `${id}.json`), JSON.stringify(doc, null, 2), "utf8");
    return index[0];
  }
  ...
  async restoreSnapshot(id) {
    const raw = await readFile7(join8(this.snapshotsPath(), `${id}.json`), "utf8");
    const parsed = JSON.parse(raw);
...
function previousMedia(input) {
  ...
  const media = input.snapshot.nodes.filter((node) => (node.kind === "image" || node.kind === "video") && node.id !== input.nodeId && hasPath(node.path)).sort((left, right) => (right.shotIndex ?? -1) - (left.shotIndex ?? -1));
  return media[0];
}
...
var GenerateReadyStore = class {
  ...
  filePath() {
    return join9(resolveOutputDir(this.outputDir), FILE3);
  }
  async read() {
    try {
  ...
  async save(brief2) {
    ...
    await writeFile6(this.filePath(), JSON.stringify({ briefs: briefs.slice(-MAX3) }, null, 2), "utf8");
    return brief2;
  }
};
...
var ProductionStageStore = class {
  ...
  async write(doc) {
    ...
    await mkdir7(resolveOutputDir(this.outputDir), { recursive: true });
...
// src/edits.ts
import { appendFile, mkdir as mkdir8, readFile as readFile10 } from "node:fs/promises";
...
`, "utf8");
  ...
  async list(limit = 20) {
    const path = await this.filePath();
    const content = await readFile10(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
    const records = [];
...
// src/tasks.ts
import { appendFile as appendFile2, mkdir as mkdir9, readFile as readFile11 } from "node:fs/promises";
...
`, "utf8");
  ...
  async list() {
    const path = await this.filePath();
    const content = await readFile11(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
    const records = [];
...
async function mockImage(ctx, prompt, size) {
  ...
  await writeFile9(path, svg, "utf8");
  return {
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join15(outDir, `${slugify(prompt)}-mock.mp4`);
...
async function extractFrames(source, outputDir, options = {}) {
  ...
  const dir = join16(resolveOutputDir(outputDir), "frames");
  ...
  for (const t of times) {
    ...
    const path = join16(dir, `${stem}-${stamp}-${t.toFixed(2)}s.png`);
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join17(resolveOutputDir(ctx.settings.outputDir), "transcripts");
  await mkdir11(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join17(dir, `${slugify(source, 24)}-${stamp}.srt`);
  await writeFile10(srtPath, srt, "utf8");
  return {
    model: ctx.capability.model,
    source,
    text,
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile12(resolve6(source));
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join17(dir, `${slugify(source, 24)}-${stamp}.txt`);
    await writeFile10(txtPath, text, "utf8");
...
import { extname as extname2, join as join18 } from "node:path";
...
async function applyGrade(input) {
  ...
  const out = join18(resolveOutputDir(input.outputDir), `${slugify(`grade-${input.look}`)}-${Date.now().toString(36)}${ext}`);
...
function outputPath(outputDir, tag, ext) {
  ...
  mkdir12(root, { recursive: true }).catch(() => {
  });
  return join19(root, `${slugify(tag)}-${Date.now().toString(36)}.${ext}`);
}
...
function segmentCachePath(outputDir, fingerprint) {
  return join20(resolveOutputDir(outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join20(resolveOutputDir(outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join20(resolveOutputDir(outputDir), `faded-${Date.now().toString(36)}.mp4`);
...
async function videoUnderstand(input) {
  ...
  await mkdir13(framesDir, { recursive: true });
...
var ProposalStore = class {
  ...
  filePath() {
    return join21(resolveOutputDir(this.outputDir), "proposals.json");
  }
  async read() {
    try {
...
var CanvasIntentStore = class {
  ...
  async write(ledger) {
    ...
    await writeFile12(this.filePath(), JSON.stringify(ledger, null, 2), "utf8");
    return ledger;
  }
  async enqueue(input) {
    const prompt = input.prompt.trim();
    ...
    if (ledger.intents.length > MAX4) ledger.intents.splice(0, ledger.intents.length - MAX4);
    await this.write(ledger);
    return intent;
  }
  async list(status) {
...
import { mkdir as mkdir16, readFile as readFile15, writeFile as writeFile13 } from "node:fs/promises";
import { join as join23 } from "node:path";
var FILE5 = "studio-open.json";
var StudioTicketStore = class {
  constructor(outputDir) {
    this.outputDir = outputDir;
  }
  filePath() {
    return join23(resolveOutputDir(this.outputDir), FILE5);
  }
  async read() {
    try {
  ...
  async write(input) {
    ...
    await writeFile13(this.filePath(), JSON.stringify(ticket), "utf8");
    return ticket;
  }
};
...
import { mkdir as mkdir17, readFile as readFile16, readdir as readdir2, rm as rm2, stat } from "node:fs/promises";
import { createWriteStream } from "node:fs";
import { Transform } from "node:stream";
import { pipeline } from "node:stream/promises";
import { join as join24, relative as relative2 } from "node:path";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join24(resolveOutputDir(outputDir), EDIT_SUBDIR);
...
var ProjectStyleStore = class {
  ...
  async set(input) {
    ...
    await writeFile14(this.filePath(), JSON.stringify(merged, null, 2), "utf8");
    return merged;
  }
  /** 生成提示词的常量块（逐字复用）。 */
  block() {
...
var TermStore = class {
  ...
  async set(entries) {
    ...
    await writeFile15(this.filePath(), JSON.stringify({ terms: ledger }, null, 2), "utf8");
    return ledger;
  }
  /** 按句命中：返回文本中出现的术语及其读法。 */
  async match(text) {
...
async function contactSheet(input) {
  ...
  const out = join27(resolveOutputDir(input.outputDir), `contact-sheet-${Date.now().toString(36)}.png`);
...
var AdapterStore = class {
  ...
  catalogPath() {
    return join28(resolveOutputDir(this.outputDir), "adapters.json");
  }
  secretsPath() {
    return join28(resolveOutputDir(this.outputDir), "adapters.secrets.json");
  }
  docPath(id) {
    return join28(resolveOutputDir(this.outputDir), "adapters", `${id}.doc.txt`);
  }
  async read() {
    try {
      const parsed = JSON.parse(await readFile19(this.catalogPath(), "utf8"));
  ...
  async readDoc(id) {
    try {
      return await readFile19(this.docPath(id), "utf8");
    } catch {
      return "";
    }
  }
  async readSecrets() {
    try {
      const parsed = JSON.parse(await readFile19(this.secretsPath(), "utf8"));
  ...
  async writeSecret(id, secret) {
    ...
    await writeFile16(this.secretsPath(), JSON.stringify(all), "utf8");
  }
  async hasSecret(id) {
  ...
  async writeCatalog(file) {
    await mkdir20(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile16(this.catalogPath(), JSON.stringify(file, null, 2), "utf8");
  }
  async upsert(record) {
  ...
  async saveDoc(id, text) {
    ...
    await mkdir20(join28(resolveOutputDir(this.outputDir), "adapters"), { recursive: true });
    await writeFile16(this.docPath(id), clipped, "utf8");
...
async function orchestrateProduction(input) {
  ...
  const run = {
    ...
    reportPath: join29(resolveOutputDir(input.outputDir), `orchestrate-${kind}.json`)
  };
  await mkdir21(resolveOutputDir(input.outputDir), { recursive: true });
  await writeFile17(run.reportPath, JSON.stringify(run, null, 2), "utf8");
  return run;
}
