...
var ProposalStore = class {
  ...
  filePath() {
    return join11(resolve8(process.cwd(), this.outputDir), "proposals.json");
  }
  async read() {
    try {
...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join12(resolve9(process.cwd(), this.outputDir), CANVAS_FILE);
  }
  async read() {
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join12(resolve9(process.cwd(), this.outputDir), `canvas.json.bak-${Date.now()}`);
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join12(resolve9(process.cwd(), this.outputDir));
...
import { mkdir as mkdir9, readFile as readFile8, readdir, rm, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join13(resolve10(process.cwd(), outputDir), EDIT_SUBDIR);
...
function registerMediaListRoute(ctx, getOutputDir) {
  ...
  return webServer.register({
    ...
    handler: async (request, response) => {
      ...
      try {
        const root = resolve10(process.cwd(), getOutputDir());
        const files = [];
