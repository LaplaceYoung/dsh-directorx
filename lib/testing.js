...
var ResearchLedger = class {
  ...
  filePath() {
    return join4(resolveOutputDir(this.outputDir), FILE);
  }
  async read() {
    try {
  ...
  async record(event) {
    ...
    await writeFile2(this.filePath(), JSON.stringify({ events: next }, null, 2), "utf8");
  }
...
import { join as join5, relative, resolve as resolve4, sep as sep3 } from "node:path";
var MAX_READ = 4e4;
...
var SkillIndex = class {
  ...
  async scan() {
    ...
    const walk = async (dir) => {
      ...
      for (const entry of entries) {
        ...
        const body = await readFile4(path, "utf8").catch(() => "");
        if (body === "") continue;
        const name = /(?:^|\n)name:\s*([a-z0-9-]+)/.exec(body)?.[1] ?? relative(this.root, dir).replaceAll(sep3, "-");
        ...
        for (const child of sibling) {
          if (child.isDirectory() && (child.name === "references" || child.name === "examples")) {
            const nested = await readdir(join5(dir, child.name)).catch(() => []);
  ...
  async read(name, file) {
    ...
    if (file !== void 0 && file.trim() !== "") {
      ...
      const path = join5(this.root, record.dir, rel);
...
var PromptCraftStore = class {
  ...
  async save(craft) {
    ...
    await writeFile3(this.filePath(), JSON.stringify({ crafts: crafts.slice(-MAX2) }, null, 2), "utf8");
    return craft;
  }
};
...
var ProductionStageStore = class {
  ...
  filePath() {
    return join7(resolveOutputDir(this.outputDir), "stage.json");
  }
  async read() {
    try {
  ...
  async write(doc) {
    ...
    await writeFile4(this.filePath(), JSON.stringify(doc, null, 2), "utf8");
    return doc;
  }
  async get() {
...
function chengpianPersonaText(mode) {
  return [
    ...
    "- \u6D41\u7A0B\u95F8\uFF1A\u5148 `directorx_brief` / `directorx_chengpian` \u2192 \u5206\u53C9\u7528 `directorx_ask` \u2192 \u5267\u672C/\u5206\u955C `directorx_confirm` \u2192 **\u7B7E\u5B57\u540E\u624D**\u843D\u753B\u5E03\u3002\u751F\u6210\u524D\u5FC5\u987B knowledge_read + skill_read +\uFF08\u5FC5\u8981\u65F6\u5916\u90E8\u8C03\u7814\uFF09+ `directorx_prompt_craft`\uFF0C\u7981\u6B62\u628A\u7528\u6237\u539F\u53E5\u5F53\u63D0\u793A\u8BCD\u3002\u9636\u6BB5\u4EA7\u7269\u5199\u5165 `directorx_stage`\u3002",
...
// src/edits.ts
import { appendFile, mkdir as mkdir5, readFile as readFile7 } from "node:fs/promises";
...
// src/tasks.ts
import { appendFile as appendFile2, mkdir as mkdir6, readFile as readFile8 } from "node:fs/promises";
...
async function pollGeneric(ctx, spec, apiKey, headers, taskId) {
  ...
  for (let attempt = 0; attempt < ctx.settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve12) => setTimeout(resolve12, ctx.settings.pollIntervalMs));
...
async function mockAudio(ctx, text) {
  ...
  const path = join10(outDir, `${slugify(text, 24)}-mock.wav`);
...
async function openaiTts(ctx, text, voice, format, instructions, speed) {
  ...
  const path = join10(outDir, `${slugify(text, 24)}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}.${ext}`);
...
async function pollModelverseTask(baseURL, apiKey, taskId, settings, signal, ledger) {
  ...
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve12) => setTimeout(resolve12, settings.pollIntervalMs));
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join14(resolveOutputDir(ctx.settings.outputDir), "transcripts");
  await mkdir8(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join14(dir, `${slugify(source, 24)}-${stamp}.srt`);
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile9(resolve6(source));
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join14(dir, `${slugify(source, 24)}-${stamp}.txt`);
...
import { extname as extname2, join as join15 } from "node:path";
...
async function applyGrade(input) {
  ...
  const out = join15(resolveOutputDir(input.outputDir), `${slugify(`grade-${input.look}`)}-${Date.now().toString(36)}${ext}`);
...
function outputPath(outputDir, tag, ext) {
  ...
  mkdir9(root, { recursive: true }).catch(() => {
  });
  return join16(root, `${slugify(tag)}-${Date.now().toString(36)}.${ext}`);
}
...
// src/characters.ts
import { mkdir as mkdir10, readFile as readFile10, writeFile as writeFile8 } from "node:fs/promises";
...
var CharacterStore = class {
  ...
  filePath() {
    return join17(resolveOutputDir(this.outputDir), "characters.json");
  }
  async read() {
    try {
...
import { copyFileSync, existsSync as existsSync4, readFileSync as readFileSync2, statSync, mkdirSync } from "node:fs";
...
function segmentCachePath(outputDir, fingerprint) {
  return join18(resolveOutputDir(outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join18(resolveOutputDir(outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join18(resolveOutputDir(outputDir), `faded-${Date.now().toString(36)}.mp4`);
...
async function videoUnderstand(input) {
  ...
  await mkdir11(framesDir, { recursive: true });
...
// src/proposals.ts
import { mkdir as mkdir12, readFile as readFile11, writeFile as writeFile9 } from "node:fs/promises";
...
var ProposalStore = class {
  ...
  filePath() {
    return join19(resolveOutputDir(this.outputDir), "proposals.json");
  }
  async read() {
    try {
  ...
  async write(ledger) {
    ...
    await writeFile9(this.filePath(), JSON.stringify(ledger, null, 2), "utf8");
    return ledger;
  }
...
// src/canvas.ts
import { mkdir as mkdir13, readFile as readFile12, writeFile as writeFile10 } from "node:fs/promises";
...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join20(resolveOutputDir(this.outputDir), CANVAS_FILE);
  }
  async read() {
    const path = this.filePath();
    const raw = await readFile12(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
  ...
  async reset() {
    const path = this.filePath();
    try {
      const existing = await readFile12(path, "utf8");
      if (existing.trim() !== "") {
        const backup = join20(resolveOutputDir(this.outputDir), `canvas.json.bak-${Date.now()}`);
        await writeFile10(backup, existing, "utf8");
      }
    } catch {
    }
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join20(resolveOutputDir(this.outputDir));
    ...
    await writeFile10(path, JSON.stringify(saved), "utf8");
    return saved;
  }
  ...
  snapshotsPath() {
    return join20(resolveOutputDir(this.outputDir), ".canvas-snapshots");
  }
  snapshotsFile() {
    return join20(this.snapshotsPath(), "index.json");
  }
  async snapshot(label) {
    ...
    await mkdir13(this.snapshotsPath(), { recursive: true });
    await writeFile10(this.snapshotsFile(), JSON.stringify(index, null, 2), "utf8");
    await writeFile10(join20(this.snapshotsPath(), `${id}.json`), JSON.stringify(doc, null, 2), "utf8");
    return index[0];
  }
  async readSnapshotsIndex() {
    try {
      const parsed = JSON.parse(await readFile12(this.snapshotsFile(), "utf8"));
  ...
  async restoreSnapshot(id) {
    const raw = await readFile12(join20(this.snapshotsPath(), `${id}.json`), "utf8");
    const parsed = JSON.parse(raw);
...
var CanvasIntentStore = class {
  ...
  filePath() {
    return join21(resolveOutputDir(this.outputDir), FILE3);
  }
  async read() {
    try {
  ...
  async enqueue(input) {
    ...
    if (ledger.intents.length > MAX3) ledger.intents.splice(0, ledger.intents.length - MAX3);
    await this.write(ledger);
    return intent;
  }
  async list(status) {
...
var FILE4 = "studio-open.json";
var StudioTicketStore = class {
  constructor(outputDir) {
    this.outputDir = outputDir;
  }
  filePath() {
    return join22(resolveOutputDir(this.outputDir), FILE4);
  }
  async read() {
    try {
  ...
  async write(input) {
    ...
    await writeFile12(this.filePath(), JSON.stringify(ticket), "utf8");
    return ticket;
  }
};
...
function parseSessionList(input) {
  ...
  for (const row of rpcItems(input)) {
    ...
    const id = typeof item.sessionId === "string" ? item.sessionId : typeof item.id === "string" ? item.id : void 0;
    if (id === void 0 || id === "") continue;
    const cwd = typeof item.cwd === "string" ? item.cwd : typeof asRecord(item.header)?.cwd === "string" ? String(asRecord(item.header)?.cwd) : void 0;
    rows.push({
      id,
      ...cwd !== void 0 ? { cwd } : {},
...
function parseWorkspaceList(input) {
  ...
  for (const row of rpcItems(input)) {
    ...
    const id = typeof item.workspaceId === "string" ? item.workspaceId : typeof item.id === "string" ? item.id : void 0;
    const sessionIds = Array.isArray(item.sessionIds) ? item.sessionIds.filter((value) => typeof value === "string" && value !== "") : [];
    rows.push({
      ...id !== void 0 ? { id } : {},
      ...path !== void 0 ? { path } : {},
      sessionIds
...
function resolveLiveSession(service, sessionId) {
  ...
  const rec = asRecord2(service);
  const binding = rec?.binding;
  if (typeof binding !== "function") return void 0;
  try {
    const bound = asRecord2(binding.call(service, sessionId));
...
function parseAskItems(value) {
  ...
  for (const raw of value) {
    ...
    const intentRec = asRecord2(rec.intent);
    items.push({
      id,
      question,
...
function parseWait(value) {
  ...
  if (rec.kind === "question") {
    const payload = asRecord2(rec.payload);
...
function dockItemsFromSnapshot(raw) {
  ...
  for (const node of nodes) {
    ...
    if (kind === "command") {
      ...
      const outcome = asRecord2(rec.outcome);
      lines.push({
        id: `cmd-${seq}`,
        kind: "tool",
        name,
  ...
  const partial = asRecord2(snap.partial);
  ...
  const waits = (Array.isArray(snap.pending) ? snap.pending : []).map(parseWait).filter((item) => item !== void 0);
  return {
    lines,
    waits,
...
function asClipPayload(value) {
  ...
  if (nodes.length === 0) return void 0;
  const edges = Array.isArray(rec.edges) ? rec.edges.flatMap((item) => {
...
import { mkdir as mkdir16, readFile as readFile15, readdir as readdir2, rm as rm2, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join23(resolveOutputDir(outputDir), EDIT_SUBDIR);
...
async function listMediaFiles(outputDir) {
  ...
  const scan = async (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      const rel = relative2(currentProjectRoot(), full);
      files.push({
...
import { mkdir as mkdir17, readFile as readFile16, writeFile as writeFile13 } from "node:fs/promises";
...
var ProjectStyleStore = class {
  ...
  filePath() {
    return join24(resolveOutputDir(this.outputDir), "style.json");
  }
  async read() {
    try {
  ...
  async set(input) {
    ...
    await writeFile13(this.filePath(), JSON.stringify(merged, null, 2), "utf8");
    return merged;
  }
  /** 生成提示词的常量块（逐字复用）。 */
  block() {
...
var TermStore = class {
  ...
  filePath() {
    return join25(resolveOutputDir(this.outputDir), "terms.json");
  }
  async read() {
    try {
  ...
  async set(entries) {
    ...
    await writeFile14(this.filePath(), JSON.stringify({ terms: ledger }, null, 2), "utf8");
    return ledger;
  }
  /** 按句命中：返回文本中出现的术语及其读法。 */
  async match(text) {
...
async function contactSheet(input) {
  ...
  const out = join26(resolveOutputDir(input.outputDir), `contact-sheet-${Date.now().toString(36)}.png`);
  ...
  const result = spawnSync7("ffmpeg", args, { encoding: "utf8" });
...
var AdapterStore = class {
  ...
  catalogPath() {
    return join27(resolveOutputDir(this.outputDir), "adapters.json");
  }
  secretsPath() {
    return join27(resolveOutputDir(this.outputDir), "adapters.secrets.json");
  }
  docPath(id) {
    return join27(resolveOutputDir(this.outputDir), "adapters", `${id}.doc.txt`);
  }
  async read() {
    try {
      const parsed = JSON.parse(await readFile18(this.catalogPath(), "utf8"));
  ...
  async readDoc(id) {
    try {
      return await readFile18(this.docPath(id), "utf8");
    } catch {
      return "";
    }
  }
  async readSecrets() {
    try {
      const parsed = JSON.parse(await readFile18(this.secretsPath(), "utf8"));
  ...
  async writeSecret(id, secret) {
    ...
    await writeFile15(this.secretsPath(), JSON.stringify(all), "utf8");
  }
  async hasSecret(id) {
  ...
  async writeCatalog(file) {
    await mkdir19(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile15(this.catalogPath(), JSON.stringify(file, null, 2), "utf8");
  }
  async upsert(record) {
  ...
  async saveDoc(id, text) {
    ...
    await mkdir19(join27(resolveOutputDir(this.outputDir), "adapters"), { recursive: true });
    await writeFile15(this.docPath(id), clipped, "utf8");
...
import { mkdir as mkdir20, writeFile as writeFile16 } from "node:fs/promises";
...
async function orchestrateProduction(input) {
  ...
  const run = {
    ...
    reportPath: join28(resolveOutputDir(input.outputDir), `orchestrate-${kind}.json`)
  };
  await mkdir20(resolveOutputDir(input.outputDir), { recursive: true });
  await writeFile16(run.reportPath, JSON.stringify(run, null, 2), "utf8");
  return run;
}
