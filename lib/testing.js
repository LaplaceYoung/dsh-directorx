...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join10(resolve7(process.cwd(), this.outputDir), CANVAS_FILE);
  }
  async read() {
    const path = this.filePath();
    const raw = await readFile6(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join10(resolve7(process.cwd(), this.outputDir));
  ...
  async update(id, patch) {
    return this.mutate((doc) => {
      ...
      if (nodeIndex >= 0) {
        doc.nodes[nodeIndex] = sanitizeNode({ ...doc.nodes[nodeIndex], ...patch, id });
        return;
      }
...
import { mkdir as mkdir7, readdir, rm, stat } from "node:fs/promises";
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join11(resolve8(process.cwd(), outputDir), EDIT_SUBDIR);
...
function registerMediaListRoute(ctx, getOutputDir) {
  ...
  return webServer.register({
    ...
    handler: async (request, response) => {
      ...
      try {
        const root = resolve8(process.cwd(), getOutputDir());
        const files = [];
      ...
      try {
        ...
        if (request.method === "PUT") {
          ...
          const doc = await store.write(
            { version: 1, updatedAt: body.updatedAt ?? 0, nodes: body.nodes ?? [], edges: body.edges ?? [] },
            expectedUpdatedAt
          );
          send(200, doc);
          return;
