  ...
  if (settings.audio.enabled) {
    disposers.push(ctx.tools.register(safeDefine({
      ...
      async execute(args, exec) {
        ...
        const signal = combinedSignal(exec.signal, settings.timeoutMs);
        return runAudio(toolContext(settings, settings.audio, signal), gate.prompt, { voice: args.voice, format: args.format, instructions: typeof args.instructions === "string" ? args.instructions : void 0, speed: typeof args.speed === "number" ? args.speed : void 0 });
      }
    })));
    disposers.push(ctx.tools.register(safeDefine({
