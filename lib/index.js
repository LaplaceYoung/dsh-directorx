...
var ProductionStageStore = class {
  ...
  filePath() {
    return join25(resolveOutputDir(this.outputDir), "stage.json");
  }
  async read() {
    try {
  ...
  async write(doc) {
    ...
    await writeFile13(this.filePath(), JSON.stringify(doc, null, 2), "utf8");
    return doc;
  }
  async get() {
    const doc = await this.read();
...
var AdapterStore = class {
  ...
  async read() {
    try {
      const parsed = JSON.parse(await readFile17(this.catalogPath(), "utf8"));
  ...
  async readDoc(id) {
    try {
      return await readFile17(this.docPath(id), "utf8");
    } catch {
      return "";
    }
  }
  async readSecrets() {
    try {
      const parsed = JSON.parse(await readFile17(this.secretsPath(), "utf8"));
  ...
  async writeSecret(id, secret) {
    ...
    await writeFile14(this.secretsPath(), JSON.stringify(all), "utf8");
  }
  async hasSecret(id) {
  ...
  async writeCatalog(file) {
    await mkdir18(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile14(this.catalogPath(), JSON.stringify(file, null, 2), "utf8");
  }
  async upsert(record) {
  ...
  async saveDoc(id, text) {
    ...
    await mkdir18(join27(resolveOutputDir(this.outputDir), "adapters"), { recursive: true });
    await writeFile14(this.docPath(id), clipped, "utf8");
...
${blocks.join("\uFF1B")}` : gate.prompt;
  ...
  disposers.push(ctx.tools.register(safeDefine({
    ...
    isConcurrencySafe: () => true,
    async execute(args) {
      return skillIndex.read(String(args.name), typeof args.file === "string" ? args.file : void 0);
    }
  })));
  disposers.push(ctx.tools.register(safeDefine({
