...
async function mockAudio(ctx, text) {
  ...
  const path = join16(outDir, `${slugify(text, 24)}-mock.wav`);
...
async function openaiTts(ctx, text, voice, format, instructions, speed) {
  ...
  const path = join16(outDir, `${slugify(text, 24)}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}.${ext}`);
...
async function mockImage(ctx, prompt, size) {
  ...
  const path = join17(outDir, name2);
  const svg = [
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join18(resolve16(process.cwd(), ctx.settings.outputDir), "transcripts");
  await mkdir13(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join18(dir, `${slugify(source, 24)}-${stamp}.srt`);
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile13(resolve16(source));
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join18(dir, `${slugify(source, 24)}-${stamp}.txt`);
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join19(outDir, `${slugify(prompt)}-mock.mp4`);
...
function segmentCachePath(outputDir, fingerprint) {
  return join20(resolve17(process.cwd(), outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join20(resolve17(process.cwd(), outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join20(resolve17(process.cwd(), outputDir), `faded-${Date.now().toString(36)}.mp4`);
...
async function videoUnderstand(input) {
  const probe = probeMedia(input.source);
  const framesDir = resolve18(process.cwd(), input.outputDir);
  await mkdir14(framesDir, { recursive: true });
...
${blocks.join("\uFF1B")}` : args.prompt;
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    timeoutMs: 15e3,
    async execute(args) {
      const status = args.status === "pending" || args.status === "taken" || args.status === "done" ? args.status : void 0;
