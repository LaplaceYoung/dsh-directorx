...
import { mkdir as mkdir5, readdir, rm, stat } from "node:fs/promises";
...
async function readSkillFile(path) {
  try {
    return parseFrontmatter(await readFile6(path, "utf8"));
  } catch {
    return void 0;
  }
}
...
import { mkdir as mkdir7, readFile as readFile7, writeFile as writeFile5 } from "node:fs/promises";
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile7(resolve9(source));
