...
async function videoConcat(input) {
  ...
  if (input.transition === "cut" || fadeSec <= 0) {
    ...
    const anyAudio = probes2.some((probe) => probe.streams.some((stream) => stream.type === "audio"));
    const args2 = [];
    const filters2 = [];
    ...
    const filterComplex2 = `${filters2.join(";")};${inputs}concat=n=${input.files.length}:v=1:a=${anyAudio ? 1 : 0}${anyAudio ? "[v][a]" : "[v]"}`;
    args2.push("-filter_complex", filterComplex2, "-map", "[v]", "-c:v", "libx264", "-preset", "veryfast");
    if (anyAudio) args2.push("-map", "[a]", "-c:a", "aac");
    args2.push(out);
...
async function mockAudio(ctx, text) {
  ...
  const path = join11(outDir, `${slugify(text, 24)}-mock.wav`);
  await writeFile4(path, makeWav());
...
async function openaiTts(ctx, text, voice, format) {
  ...
  const path = join11(outDir, `${slugify(text, 24)}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}.${ext}`);
...
async function mockImage(ctx, prompt, size) {
  ...
  const path = join12(outDir, name2);
  const svg = [
  ...
  await writeFile5(path, svg, "utf8");
  return {
    model: ctx.capability.model,
    prompt,
...
import { mkdir as mkdir9, readFile as readFile9, writeFile as writeFile6 } from "node:fs/promises";
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join13(resolve11(process.cwd(), ctx.settings.outputDir), "transcripts");
  await mkdir9(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join13(dir, `${slugify(source, 24)}-${stamp}.srt`);
  await writeFile6(srtPath, srt, "utf8");
  return {
    model: ctx.capability.model,
    source,
    text,
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile9(resolve11(source));
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join13(dir, `${slugify(source, 24)}-${stamp}.txt`);
    await writeFile6(txtPath, text, "utf8");
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join14(outDir, `${slugify(prompt)}-mock.mp4`);
  const ffmpeg = spawnSync3("ffmpeg", [
    "-y",
    "-f",
    "lavfi",
    "-i",
...
var CharacterStore = class {
  ...
  filePath() {
    return join15(resolve12(process.cwd(), this.outputDir), "characters.json");
  }
  async read() {
    try {
  ...
  async register(input) {
    ...
    const card = { name: name2, description: (input.description ?? "").slice(0, 1e3), refPath: input.refPath, at: Date.now() };
    ...
    await mkdir10(resolve12(process.cwd(), this.outputDir), { recursive: true });
...
async function videoUnderstand(input) {
  const probe = probeMedia(input.source);
  const framesDir = resolve13(process.cwd(), input.outputDir);
...
\u89D2\u8272\u4E00\u81F4\u6027\u951A\u70B9\uFF1A${characterNote}` : args.prompt;
        return runVideo(toolContext(settings, settings.video, signal), prompt, {
          ...
          referenceImagePaths: refs,
          negativePrompt: typeof args.negative_prompt === "string" ? args.negative_prompt : void 0
        });
      }
    })));
  }
