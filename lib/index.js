...
function segmentCachePath(outputDir, fingerprint) {
  return join18(resolve15(process.cwd(), outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join18(resolve15(process.cwd(), outputDir), ".timeline-cache"), { recursive: true });
