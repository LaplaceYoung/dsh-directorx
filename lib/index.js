...
var VIDEO_MODES = ["openai-videos", "modelverse-tasks", "kling", "kling-v3", "runway", "minimax-h3", "vidu", "mock"];
...
async function videoUnderstand(input) {
  ...
  const framesDir = resolve12(process.cwd(), input.outputDir);
  await mkdir10(framesDir, { recursive: true });
...
// src/characters.ts
import { mkdir as mkdir11, readFile as readFile10, writeFile as writeFile7 } from "node:fs/promises";
...
var CharacterStore = class {
  ...
  filePath() {
    return join16(resolve13(process.cwd(), this.outputDir), "characters.json");
  }
  async read() {
    try {
