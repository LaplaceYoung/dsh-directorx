...
var SKILL_ARTICLES = {
  "directorx-methodology": ["01", "02", "07", "123", "127", "213"],
  ...
  "novel-storyboard": ["172", "01", "109"],
  "novel-art": ["126", "228", "226"],
  "storyboard-craft": ["172", "01", "109"],
  "trailer-craft": ["151", "205", "188", "01"],
  "directorx-series-craft": ["117", "123", "01"],
  "directorx-blocking-craft": ["117", "123", "116"],
  "editing-workflow": ["02", "127", "15"],
  "frame-qa": ["174", "118", "111"],
  "video-prompt-builder": ["115", "130", "158", "01", "213"],
  ...
  "cinematic-style": ["126", "01", "09"],
  "continuous-video": ["116", "117", "123"],
...
function articlesForSkill(name2) {
  return SKILL_ARTICLES[name2] ?? [];
}
...
function articlesForSkills(names) {
  return unique(names.flatMap((name2) => articlesForSkill(name2)));
}
...
function matchArticle(inventory, wanted) {
  const hits = inventory.filter((article) => article.id === wanted || article.slug === wanted || String(article.number) === wanted || (article.aliases ?? []).includes(wanted));
...
import { mkdir as mkdir13, readFile as readFile12, readdir, rm, stat } from "node:fs/promises";
...
var IpMemoryStore = class {
  ...
  async write(entries) {
    ...
    await writeFile3(this.filePath(), JSON.stringify({ entries: next }, null, 2), "utf8");
  }
  ...
  async recall(text) {
    ...
    const ranked = (await this.read()).filter((entry) => overlap(entry.terms, needles) || overlap(entry.exclude, needles)).sort((left, right) => right.uses - left.uses || right.at - left.at);
...
function toolsForSkill(name2) {
  return SKILL_TOOLS[name2] ?? [];
}
...
function withIpRoute(intent, route) {
  ...
  return {
    ...route,
    articles: unique2(["213", ...route.articles]),
    knowledge: unique2(["\u7248\u6743\u5B89\u5168 \u6CDB\u5316 213", ...route.knowledge]),
    tools: unique2(["directorx_ip_scan", "directorx_ip_rewrite", ...route.tools]),
    next: unique2(["directorx_ip_scan", "directorx_knowledge_read 213", ...route.next]),
    avoid: unique2([
      ...route.avoid,
...
function routeSkills(intent) {
  ...
  if (rule === void 0) {
    return withIpRoute(text, { ...DEFAULT_ROUTE, knowledge: unique2([text, ...DEFAULT_ROUTE.knowledge]) });
  }
  const tools = unique2([
    ...rule.skills.flatMap((name2) => toolsForSkill(name2)),
    ...rule.extraTools ?? []
  ]);
  const articles = unique2([...articlesForSkills(rule.skills), ...articlesForMode(rule.mode)]);
  const next = [
    ...rule.skills.map((name2) => `directorx_skill_read ${name2}`),
...
function planPrompt(input) {
  ...
  const next = [
    ...route.skills.slice(0, 3).map((name2) => `directorx_skill_read ${name2}`),
...
function formatDshCanvasPrompt(intent, extras = {}) {
  ...
  return [
    ...
    plan.agentPrompt,
    /片段重做|重做中段/.test(intent.prompt) ? '- \u8FD9\u662F\u7247\u6BB5\u91CD\u505A\uFF1A\u751F\u6210\u53EA\u8986\u76D6\u4E2D\u6BB5\uFF0C\u56DE\u5199\u8BE5\u89C6\u9891\u8282\u70B9 path \u540E\u8C03\u7528 directorx_canvas_reshoot { action: "assemble", nodeId: \u4E2D\u6BB5id } \u628A\u5934+\u4E2D+\u5C3E\u62FC\u56DE\u6210\u7247\u3002' : "",
    /拼成片|硬切组装|预告片/.test(intent.prompt) ? "- \u8FD9\u662F\u6210\u7247\u7EC4\u88C5\uFF1A\u8C03\u7528 directorx_canvas_pack\uFF0Ctransition=cut\u3002\u9884\u544A\u7247\u7981\u6B62 fade\u3002" : "",
    /镜改|再生动|改这一镜|只改这/.test(intent.prompt) ? "- \u8FD9\u662F\u5355\u955C\u4FEE\u6539\uFF1A\u5148 directorx_revise { nodeId: \u6E90\u8282\u70B9 }\uFF0C\u6210\u7A3F\u53EA\u6539\u8FD9\u4E00\u5904\u3002\u56DE\u5199\u8BE5\u8282\u70B9 path\u3002\u6574\u677F\u5176\u5B83\u955C\u4E0D\u8981\u91CD\u505A\u3002\u7CFB\u5217\u5305\u5DF2\u6FC0\u6D3B\u5C31\u6CBF\u7528\uFF0C\u4E0D\u8981\u8BA9\u7528\u6237\u518D\u62A5\u4EBA\u8BBE\u3002" : "",
    ...
    /宫格拼回|分屏对照|去硬字|续写位|导出动图/.test(intent.prompt) ? "- \u8FD9\u662F\u753B\u5E03\u5DE5\u827A\uFF1A\u62FC\u56DE directorx_canvas_join\uFF0C\u5206\u5C4F directorx_canvas_stack\uFF0C\u53BB\u5B57 directorx_canvas_desub\uFF0C\u7EED\u5199\u4F4D directorx_canvas_extend\uFF08\u4E0D\u751F\u6210\uFF09\uFF0C\u52A8\u56FE directorx_canvas_gif\u3002\u90FD\u4E0D\u8981\u8D70 generate\u3002" : "",
...
var CanvasIntentStore = class {
  ...
  filePath() {
    return join6(resolveOutputDir(this.outputDir), FILE2);
  }
  async read() {
    try {
  ...
  async enqueue(input) {
    ...
    if (ledger.intents.length > MAX2) ledger.intents.splice(0, ledger.intents.length - MAX2);
    await this.write(ledger);
    return intent;
  }
  async list(status) {
...
var CharacterStore = class {
  ...
  filePath() {
    return join7(resolveOutputDir(this.outputDir), "characters.json");
  }
  async read() {
    try {
...
// src/edits.ts
import { appendFile, mkdir as mkdir6, readFile as readFile7 } from "node:fs/promises";
...
`, "utf8");
  ...
  async list() {
    const path = await this.filePath();
    const content = await readFile8(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
    const records = [];
...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join10(resolveOutputDir(this.outputDir), CANVAS_FILE);
  }
  async read() {
    const path = this.filePath();
    const raw = await readFile9(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
  ...
  async reset() {
    ...
    try {
      ...
      if (existing.trim() !== "") {
        const backup = join10(resolveOutputDir(this.outputDir), `canvas.json.bak-${Date.now()}`);
        await writeFile6(backup, existing, "utf8");
      }
    } catch {
    }
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join10(resolveOutputDir(this.outputDir));
    ...
    await writeFile6(path, JSON.stringify(saved), "utf8");
    return saved;
  }
  ...
  snapshotsPath() {
    return join10(resolveOutputDir(this.outputDir), ".canvas-snapshots");
  }
  snapshotsFile() {
    return join10(this.snapshotsPath(), "index.json");
  }
  async snapshot(label) {
    ...
    await mkdir8(this.snapshotsPath(), { recursive: true });
    await writeFile6(this.snapshotsFile(), JSON.stringify(index, null, 2), "utf8");
    await writeFile6(join10(this.snapshotsPath(), `${id}.json`), JSON.stringify(doc, null, 2), "utf8");
    return index[0];
  }
  async readSnapshotsIndex() {
    try {
      const parsed = JSON.parse(await readFile9(this.snapshotsFile(), "utf8"));
  ...
  async restoreSnapshot(id) {
    const raw = await readFile9(join10(this.snapshotsPath(), `${id}.json`), "utf8");
    const parsed = JSON.parse(raw);
...
async function applyGrade(input) {
  ...
  const out = join11(resolveOutputDir(input.outputDir), `${slugify(`grade-${input.look}`)}-${Date.now().toString(36)}${ext}`);
...
import { mkdir as mkdir9, readFile as readFile10, writeFile as writeFile7 } from "node:fs/promises";
import { join as join12 } from "node:path";
var FILE3 = "studio-open.json";
var StudioTicketStore = class {
  constructor(outputDir) {
    this.outputDir = outputDir;
  }
  filePath() {
    return join12(resolveOutputDir(this.outputDir), FILE3);
  }
  async read() {
    try {
  ...
  async write(input) {
    ...
    await writeFile7(this.filePath(), JSON.stringify(ticket), "utf8");
    return ticket;
  }
};
...
function planAutolink(doc, options = {}) {
  ...
  for (const card of options.characters ?? []) {
    ...
    const anchors = pool.filter((node) => {
      ...
      return node.label.includes(name2) || (node.characters ?? []).includes(name2);
    });
    if (anchors.length === 0) continue;
    for (const target of pool) {
      if (target.kind !== "image" && target.kind !== "video") continue;
      if (!mentions(blobOf(target), name2) && !(target.characters ?? []).includes(name2)) continue;
...
async function extractFrames(source, outputDir, options = {}) {
  ...
  const dir = join13(resolveOutputDir(outputDir), "frames");
  ...
  for (const t of times) {
    ...
    const path = join13(dir, `${stem}-${stamp}-${t.toFixed(2)}s.png`);
...
function resolveLocalVideo(outputDir, candidate) {
  if (candidate !== "" && existsSync3(candidate)) return candidate;
  const resolved = resolveMediaPath(outputDir, candidate);
  if (!existsSync3(resolved)) throw new Error(`\u627E\u4E0D\u5230\u89C6\u9891\u6587\u4EF6\uFF1A${candidate}`);
  return resolved;
}
...
import { existsSync as existsSync5, renameSync, rmSync } from "node:fs";
...
function requireFfmpeg() {
  const found = spawnSync3("which", ["ffmpeg"], { encoding: "utf8" });
  return found.status === 0 && found.stdout.trim() !== "";
}
function runVf(source, dest, vf, what) {
  if (!existsSync4(source)) throw new Error(`${what}: missing ${source}`);
  if (!requireFfmpeg()) throw new Error(`${what}: ffmpeg not on PATH`);
  const result = spawnSync3("ffmpeg", ["-hide_banner", "-y", "-i", source, "-vf", vf, dest], { encoding: "utf8" });
  if (result.status !== 0 || !existsSync4(dest)) {
...
function ensureAspectFrame(source, outputDir, aspectW = 16, aspectH = 9) {
  if (!existsSync4(source)) throw new Error(`ensureAspectFrame: missing ${source}`);
  if (!requireFfmpeg()) return source;
  const dest = join14(resolveOutputDir(outputDir), `${slugify("frame-fit")}-${aspectW}x${aspectH}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}.png`);
  try {
...
function runFfmpeg(args, what) {
  ...
  const result = spawnSync4("ffmpeg", ["-hide_banner", "-y", ...finalArgs], { encoding: "utf8" });
  if (result.status !== 0) {
...
async function videoProcess(input) {
  ...
  if (input.lut3d !== void 0 && input.lut3d !== "") {
    if (!existsSync5(input.lut3d)) throw new Error(`LUT \u6587\u4EF6\u4E0D\u5B58\u5728\uFF1A${input.lut3d}`);
...
function hasLibass() {
  ...
  const result = spawnSync4("ffmpeg", ["-hide_banner", "-h", "filter=ass"], { encoding: "utf8" });
...
function ledgerPath(outputDir) {
  return join16(resolveOutputDir(outputDir), "reshoots.json");
}
async function readJobs(outputDir) {
  try {
    const parsed = JSON.parse(await readFile11(ledgerPath(outputDir), "utf8"));
...
async function writeJobs(outputDir, jobs) {
  await mkdir12(resolveOutputDir(outputDir), { recursive: true });
  await writeFile8(ledgerPath(outputDir), JSON.stringify({ jobs: jobs.slice(-40) }, null, 2), "utf8");
}
...
function parseStructured(lines, cap) {
  ...
  for (const raw of lines) {
    ...
    if (spoken !== null) {
      const name2 = (spoken[1] ?? "").trim();
      const speech = (spoken[2] ?? "").trim();
      const beat = ensureBeat();
      if (name2 !== "") beat.characters.push(name2);
      beat.prompt.push(speech === "" ? name2 : `${name2}\uFF1A${speech}`);
      last = speech === "" ? "character" : "other";
      continue;
    }
    if (CHAR_EN.test(line) && !SCENE_EN.test(line) && !TRANSITION.test(line)) {
      const name2 = line.replace(/\s*\([^)]*\)\s*$/, "").trim();
      const beat = ensureBeat();
      beat.characters.push(name2);
      last = "character";
      continue;
    }
...
function uniqueNames(names) {
  ...
  for (const name2 of names) {
    const trimmed = name2.trim().slice(0, 80);
...
async function contactSheet(input) {
  ...
  const out = join17(resolveOutputDir(input.outputDir), `contact-sheet-${Date.now().toString(36)}.png`);
...
async function applyGridSplit(input) {
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < cols; col += 1) {
      const out = join18(dir, `grid-${source.id}-${row}-${col}-${Date.now().toString(36)}.png`);
...
function runFfmpeg2(args, what) {
  const result = spawnSync8("ffmpeg", ["-hide_banner", "-y", ...args], { encoding: "utf8" });
...
function tileStills(files, outputDir, columns, numbered) {
  ...
  const out = join19(resolveOutputDir(outputDir), `grid-join-${Date.now().toString(36)}.png`);
  const attempt = (withNumbers) => {
    const result = spawnSync8("ffmpeg", [
      "-hide_banner",
      "-y",
      ...args,
      "-filter_complex",
...
async function applySplitScreen(input) {
  ...
  const out = join19(resolveOutputDir(input.outputDir), `stack-${Date.now().toString(36)}.mp4`);
  runFfmpeg2([
    ...args,
    "-filter_complex",
    parts.join(";"),
...
async function applyDesub(input) {
  ...
  if (method === "blur") {
    ...
    const out = join19(resolveOutputDir(input.outputDir), `desub-${Date.now().toString(36)}.mp4`);
...
async function applyGifExport(input) {
  ...
  const file = resolveLocalVideo(input.outputDir, source.path);
  const out = join19(resolveOutputDir(input.outputDir), `gif-${Date.now().toString(36)}.gif`);
  runFfmpeg2([
    "-i",
    file,
    "-vf",
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join20(resolveOutputDir(outputDir), EDIT_SUBDIR);
...
function registerVendorRoute(ctx) {
  ...
  const serve = async (name2, contentType, response) => {
    try {
      const dir = vendorDirs.find((candidate) => existsSync6(join20(candidate, name2)));
      if (dir === void 0) throw new Error(`vendor asset ${name2} missing`);
      const data = await readFile12(join20(dir, name2));
      response.writeHead(200, {
...
// src/skills.ts
import { readdir as readdir2, readFile as readFile13 } from "node:fs/promises";
...
async function readSkillFile(path) {
  try {
    return parseFrontmatter(await readFile13(path, "utf8"));
  } catch {
    return void 0;
  }
}
async function firstLevelSkillDirs(root) {
  ...
  for (const entry of entries) {
    ...
    const dir = join21(root, entry.name);
...
async function registerBundledSkills(ctx) {
  ...
  const root = resolve8(skillsRoot);
  for (const { dir, safeName } of await firstLevelSkillDirs(root)) {
    const parsed = await readSkillFile(join21(dir, "SKILL.md"));
  ...
  const cnRoot = join21(root, "video-production-cn");
  ...
  ctx.skills.register({
    ...
    content: [
      ...
      "- \u7EC4\u88C5\u4F18\u5148\u786E\u5B9A\u6027\uFF1Adirectorx_edit_plan \u8DEF\u7531\u540E directorx_image_edit / directorx_video_process / directorx_edit + directorx_video_concat\uFF08xfade \u6210\u7247\uFF09\uFF0C\u4EA7\u7269\u8DEF\u5F84\u5199\u56DE\u753B\u5E03\u3002",
      "",
      "## \u753B\u5E03\u955C\u50CF\uFF08\u5FC5\u987B\uFF09",
      "",
...
async function klingVideo(ctx, prompt, options) {
  ...
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve14) => setTimeout(resolve14, settings.pollIntervalMs));
...
var MODES = ["\u4E25\u683C", "\u81EA\u52A8", "\u534F\u540C"];
var VARIANT_LENSES = [
  { shotSize: "LS", lighting: "soft-window", composition: "depth-layers", cameraMove: "static", label: "\u5EFA\u7ACB\u955C\u5934", hint: "\u8FDC\u666F\u4EA4\u4EE3\u7A7A\u95F4\u5C42\u6B21\uFF0C\u67D4\u7A97\u5149\uFF0C\u7EB5\u6DF1\u4E09\u5C42" },
  { shotSize: "MCU", lighting: "rembrandt", composition: "rule-of-thirds", cameraMove: "push_in", label: "\u4EBA\u7269\u8FD1\u903C", hint: "\u4E2D\u8FD1\u666F\u63A8\u8FD1\uFF0C\u4FA7\u5149\u5851\u8138\uFF0C\u4E09\u5206\u6784\u56FE" },
  { shotSize: "CU", lighting: "low-key", composition: "negative-space", cameraMove: "static", label: "\u60C5\u7EEA\u7279\u5199", hint: "\u7279\u5199\u9501\u53CD\u5E94\uFF0C\u4F4E\u8C03\u5149\uFF0C\u7559\u8D1F\u7A7A\u95F4" },
  { shotSize: "MS", lighting: "golden-hour", composition: "symmetry", cameraMove: "pan", label: "\u52A8\u4F5C\u4E2D\u666F", hint: "\u4E2D\u666F\u8DDF\u52A8\u4F5C\uFF0C\u9EC4\u91D1\u65F6\u523B\uFF0C\u5BF9\u79F0\u6216\u6A2A\u79FB" }
];
...
function draftDirectorPrompts(task, count = 3) {
  ...
  return VARIANT_LENSES.slice(0, n).map((lens) => `\u3010${lens.label}\u3011${subject}\u3002\u6309 ${lens.shotSize}/${lens.lighting}/${lens.composition} \u5199\uFF1A${lens.hint}\u3002\u672C\u884C\u662F\u89D2\u5EA6\u4E0D\u662F\u6210\u7A3F\uFF0C\u5148 directorx_prompt_plan \u518D prompt_craft\u3002`);
}
...
import { mkdir as mkdir14, readFile as readFile14, writeFile as writeFile9 } from "node:fs/promises";
...
var ProjectStyleStore = class {
  ...
  filePath() {
    return join22(resolveOutputDir(this.outputDir), "style.json");
  }
  async read() {
    try {
  ...
  async set(input) {
    ...
    await mkdir14(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile9(this.filePath(), JSON.stringify(merged, null, 2), "utf8");
    return merged;
  }
  /** 生成提示词的常量块（逐字复用）。 */
  block() {
...
// src/terms.ts
import { mkdir as mkdir15, readFile as readFile15, writeFile as writeFile10 } from "node:fs/promises";
...
var TermStore = class {
  ...
  filePath() {
    return join23(resolveOutputDir(this.outputDir), "terms.json");
  }
  async read() {
    try {
  ...
  async set(entries) {
    ...
    await mkdir15(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile10(this.filePath(), JSON.stringify({ terms: ledger }, null, 2), "utf8");
    return ledger;
  }
  /** 按句命中：返回文本中出现的术语及其读法。 */
  async match(text) {
...
async function orchestrateProduction(input) {
  ...
  const run = {
    ...
    reportPath: join24(resolveOutputDir(input.outputDir), `orchestrate-${kind}.json`)
  };
  await mkdir16(resolveOutputDir(input.outputDir), { recursive: true });
  await writeFile11(run.reportPath, JSON.stringify(run, null, 2), "utf8");
  return run;
}
...
function resolveLocalMedia(outputDir, candidate) {
  ...
  try {
    ...
    if (isAbsolute2(trimmed) && existsSync7(trimmed)) return trimmed;
    throw error;
  }
}
async function resolveBoundMedia(input) {
  ...
  if (!existsSync7(source)) throw new Error(`\u5A92\u4F53\u4E0D\u5B58\u5728\uFF1A${source}`);
...
async function commitBoundMedia(input) {
  ...
  const bytes = existsSync7(input.path) ? statSync(input.path).size : 0;
  await input.ledger.append({
    at: Date.now(),
    path: input.path,
    mediaType,
    bytes,
    name: basename2(input.path)
  });
...
async function pollGeneric(ctx, spec, apiKey, headers, taskId) {
  ...
  for (let attempt = 0; attempt < ctx.settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve14) => setTimeout(resolve14, ctx.settings.pollIntervalMs));
...
async function mockAudio(ctx, text) {
  ...
  const path = join25(outDir, `${slugify(text, 24)}-mock.wav`);
  await writeFile12(path, makeWav());
...
async function openaiTts(ctx, text, voice, format, instructions, speed) {
  ...
  const path = join25(outDir, `${slugify(text, 24)}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}.${ext}`);
  await writeFile12(path, bytes);
...
import { basename as basename3, join as join26, resolve as resolve10 } from "node:path";
...
async function mockImage(ctx, prompt, size) {
  ...
  const path = join26(outDir, name2);
  const svg = [
  ...
  await writeFile13(path, svg, "utf8");
  return {
    model: ctx.capability.model,
    prompt,
...
function resolveLocalImage(source) {
  if (existsSync8(source)) return source;
  const project = currentProjectRoot();
  const hit = [resolve10(source), resolve10(project, source)].find((candidate) => existsSync8(candidate));
...
async function openaiImageEdit(ctx, prompt, size, quality, referenceImagePaths) {
  ...
  for (const source of referenceImagePaths.slice(0, 4)) {
    ...
    const bytes = await readFile16(path);
    blobs.push({ blob: new Blob([new Uint8Array(bytes)], { type: mimeForPath(path) }), name: basename3(path) });
  }
...
import { extname as extname3, join as join27 } from "node:path";
...
async function imageProcess(input) {
  if (!existsSync9(input.source)) throw new Error(`\u56FE\u7247\u4E0D\u5B58\u5728\uFF1A${input.source}`);
  if (!hasImageOp(input)) throw new Error("\u6CA1\u6709\u53EF\u6267\u884C\u7684\u56FE\u7247\u64CD\u4F5C\uFF08\u65CB\u8F6C/\u7FFB\u8F6C/\u88C1\u5207/\u7F29\u653E/\u660E\u6697/\u8C03\u8272\uFF09");
  const which = spawnSync9("which", ["ffmpeg"], { encoding: "utf8" });
  if (which.status !== 0 || which.stdout.trim() === "") {
  ...
  const out = join27(resolveOutputDir(input.outputDir), `${slugify("image-edit")}-${Date.now().toString(36)}.${ext}`);
  const result = spawnSync9("ffmpeg", ["-hide_banner", "-y", "-i", input.source, "-vf", filters.join(","), out], { encoding: "utf8" });
  if (result.status !== 0 || !existsSync9(out)) {
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join28(resolveOutputDir(ctx.settings.outputDir), "transcripts");
  await mkdir17(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join28(dir, `${slugify(source, 24)}-${stamp}.srt`);
  await writeFile14(srtPath, srt, "utf8");
  return {
    model: ctx.capability.model,
    source,
    text,
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile17(resolve11(source));
  const form = new FormData();
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join28(dir, `${slugify(source, 24)}-${stamp}.txt`);
    await writeFile14(txtPath, text, "utf8");
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join29(outDir, `${slugify(prompt)}-mock.mp4`);
  const ffmpeg = spawnSync10("ffmpeg", [
    "-y",
    "-f",
    "lavfi",
    "-i",
...
import { copyFileSync, existsSync as existsSync10, readFileSync as readFileSync2, statSync as statSync2, mkdirSync } from "node:fs";
...
function segmentCachePath(outputDir, fingerprint) {
  return join30(resolveOutputDir(outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
async function renderTimeline(spec, outputDir) {
  ...
  for (const [index, scene] of spec.scenes.entries()) {
    if (scene.source === "" || !existsSync10(scene.source)) {
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join30(resolveOutputDir(outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join30(resolveOutputDir(outputDir), `faded-${Date.now().toString(36)}.mp4`);
        ...
        const result = spawnSync11("ffmpeg", fargs, { encoding: "utf8" });
...
async function audioSync(input) {
  const steps = [];
  const detect = spawnSync11("ffmpeg", [
    "-hide_banner",
    "-i",
    input.narration,
    "-af",
...
async function smartCut(input) {
  ...
  for (const sentence of script) {
    ...
    for (const cue of cues) {
      const score = overlap2(sentence, cue.text);
      if (score > bestScore) {
        bestScore = score;
        best = cue;
      }
...
async function videoUnderstand(input) {
  ...
  await mkdir18(framesDir, { recursive: true });
...
var ResearchLedger = class {
  ...
  filePath() {
    return join31(resolveOutputDir(this.outputDir), FILE4);
  }
  async read() {
    try {
  ...
  async record(event) {
    ...
    const next = events.slice(-MAX3);
    await mkdir19(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile15(this.filePath(), JSON.stringify({ events: next }, null, 2), "utf8");
  }
  ...
  async hasReads(input) {
    ...
    for (const name2 of input.skills) {
      if (![...skills].some((item2) => item2 === name2 || item2.includes(name2))) missing.push(`skill:${name2}`);
    }
...
// src/prompt-craft.ts
import { mkdir as mkdir20, readFile as readFile20, writeFile as writeFile16 } from "node:fs/promises";
import { join as join33 } from "node:path";

// src/skill-index.ts
import { readdir as readdir3, readFile as readFile19 } from "node:fs/promises";
import { join as join32, relative as relative2, resolve as resolve13, sep as sep3 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
var MAX_READ = 4e4;
var expandQuery2 = expandCraftQuery;
var tokens = textTokens;
var overlap3 = overlapScore;
var SkillIndex = class {
  root = resolve13(process.cwd(), "skills");
  extraRoots = [];
  cache;
  setRoot(root) {
    this.root = resolve13(root);
    this.cache = void 0;
  }
  setExtraRoots(roots) {
    this.extraRoots = [...new Set(roots.map((item2) => resolve13(item2)))];
    this.cache = void 0;
  }
  invalidate() {
    this.cache = void 0;
  ...
  async scan() {
    ...
    const walkRoot = async (root) => {
      const walk = async (dir) => {
        ...
        for (const entry of entries) {
          ...
          const body = await readFile19(path, "utf8").catch(() => "");
          if (body === "") continue;
          ...
          for (const child of sibling) {
            if (child.isDirectory() && (child.name === "references" || child.name === "examples")) {
              const nested = await readdir3(join32(dir, child.name)).catch(() => []);
    ...
    for (const extra of this.extraRoots) {
      if (resolve13(extra) === this.root) continue;
      await walkRoot(extra);
    }
    return records;
  }
  async isBundledName(name2) {
  ...
  async search(query, maxResults = 8) {
    ...
    for (const record of records) {
      ...
      let score = overlap3(queryTokens, tokens(hay));
  ...
  async read(name2, file) {
    ...
    if (file !== void 0 && file.trim() !== "") {
      ...
      const path = join32(record.absDir, rel);
...
var PromptCraftStore = class {
  ...
  filePath() {
    return join33(resolveOutputDir(this.outputDir), FILE5);
  }
  async read() {
    try {
      const parsed = JSON.parse(await readFile20(this.filePath(), "utf8"));
  ...
  async save(craft) {
    ...
    await writeFile16(this.filePath(), JSON.stringify({ crafts: crafts.slice(-MAX4) }, null, 2), "utf8");
    return craft;
  }
};
...
async function craftPrompt(input) {
  ...
  if (input.knowledgeRefs.length === 0 || input.skillNames.length === 0) {
    ...
    let suggestedSkills = routed.skills.map((name2) => ({ name: name2, snippet: "route" }));
  ...
  let negative;
  const h3Craft = input.kind === "video" && input.skillNames.some((name2) => /minimax-h3|hailuo/i.test(name2));
  if (h3Craft) {
...
// src/stage.ts
import { mkdir as mkdir21, readFile as readFile21, writeFile as writeFile17 } from "node:fs/promises";
...
var ProductionStageStore = class {
  ...
  filePath() {
    return join34(resolveOutputDir(this.outputDir), "stage.json");
  }
  async read() {
    try {
      const parsed = JSON.parse(await readFile21(this.filePath(), "utf8"));
  ...
  async write(doc) {
    ...
    await writeFile17(this.filePath(), JSON.stringify(doc, null, 2), "utf8");
    return doc;
  }
  async get() {
    const doc = await this.read();
...
import { mkdir as mkdir22, readFile as readFile22, writeFile as writeFile18 } from "node:fs/promises";
import { join as join35 } from "node:path";
var FILE6 = "generate-ready.json";
var MAX5 = 80;
...
function snapshotHasCharacterRef(names, snapshot) {
  return names.some((name2) => {
    const card = snapshot.characters.find((item2) => item2.name === name2);
...
function resolveNeed(need, input, names) {
  ...
  if (need === "character-sheet") {
    ...
    const missing = [];
    for (const name2 of names) {
      const card = input.snapshot.characters.find((item2) => item2.name === name2);
...
var GenerateReadyStore = class {
  ...
  filePath() {
    return join35(resolveOutputDir(this.outputDir), FILE6);
  }
  async read() {
    try {
  ...
  async save(brief2) {
    ...
    briefs.push(brief2);
    await mkdir22(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile18(this.filePath(), JSON.stringify({ briefs: briefs.slice(-MAX5) }, null, 2), "utf8");
    return brief2;
  }
};
...
// src/notes.ts
import { mkdir as mkdir23, readFile as readFile23, writeFile as writeFile19 } from "node:fs/promises";
import { join as join36 } from "node:path";
var FILE7 = "notes.json";
var MAX6 = 80;
var NoteStore = class {
  constructor(outputDir) {
    this.outputDir = outputDir;
  ...
  filePath() {
    return join36(resolveOutputDir(this.outputDir), FILE7);
  }
  async read() {
    try {
  ...
  async append(input) {
    ...
    const notes = [...await this.read(), note].slice(-MAX6);
    await mkdir23(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile19(this.filePath(), JSON.stringify({ notes }, null, 2), "utf8");
    return note;
  }
};
...
function validSkillName2(name2) {
  return name2.length >= 2 && name2.length <= 64 && SKILL_NAME_RE.test(name2);
}
function slugSkillName(raw) {
  const latin = raw.trim().toLowerCase().replace(/['’]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48);
  if (!validSkillName2(latin)) return "";
...
function dshHome() {
  return process.env.DSH_HOME ?? join37(homedir2(), ".dsh");
}
function userSkillRoot(override) {
  return override ?? join37(dshHome(), "skills");
}
function projectSkillRoot(outputDir) {
  return join37(resolveOutputDir(outputDir), "skills");
}
...
function harvestPrompts(title, name2) {
  return {
    agentPrompt: [
      "\u6210\u7247\u5DF2\u5230\u4EA4\u4ED8\u3002\u7ACB\u523B\u7528\u63D0\u95EE\u5361\u95EE\u7528\u6237\u8981\u4E0D\u8981\u628A\u8FD9\u6B21\u6D41\u7A0B\u6536\u6210\u6280\u80FD\uFF0C\u7981\u6B62\u5728\u6B63\u6587\u5199 1.2.3 \u83DC\u5355\u3002",
      `\u8C03\u7528 directorx_skill_capture { action: "offer", present: true }\uFF0C\u9ED8\u8BA4\u540D\u662F\u300C${title}\u300D/ ${name2}\u3002`,
...
async function isReservedSkillName(name2) {
  if (RUNTIME_RESERVED.has(name2)) return true;
  return skillIndex.isBundledName(name2);
}
async function assertWritableSkillName(name2) {
  if (!validSkillName2(name2)) {
    throw new Error(`\u6280\u80FD\u540D\u5FC5\u987B\u662F 2\u201364 \u4F4D\u5C0F\u5199\u82F1\u6587\u77ED\u6A2A\u7EBF\uFF0C\u6536\u5230 "${name2}"`);
  }
  if (await isReservedSkillName(name2)) {
    throw new Error(`"${name2}" \u662F\u63D2\u4EF6\u81EA\u5E26\u6280\u80FD\uFF0C\u4E0D\u80FD\u8986\u76D6\u3002\u6362\u4E00\u4E2A dx- \u524D\u7F00\u7684\u540D\u5B57\u3002`);
  }
  return name2;
}
async function saveCapturedSkill(input) {
  const name2 = await assertWritableSkillName(input.name.trim());
  const body = stripFrontmatter(input.body).trim();
  if (body.length < 160) {
    throw new Error("\u5148\u6839\u636E harvest \u5199\u6EE1 SKILL.md \u6B63\u6587\uFF08\u6D41\u7A0B + \u7528\u6237\u4FEE\u6539\u7EAA\u5F8B\uFF09\uFF0C\u518D save\u3002\u4E0D\u8981\u4EA4\u7A7A\u58F3\u3002");
  }
  const title = input.title.trim().slice(0, 40) || name2;
  const description = (input.description?.trim() || `\u628A\u300C${title}\u300D\u8FD9\u7C7B\u6210\u7247\u6309\u5DF2\u9A8C\u8BC1\u6D41\u7A0B\u505A\u5B8C\u3002\u7528\u6237\u518D\u8BF4\u540C\u7C7B\u9898\u6750\u3001\u540C\u6837\u6539\u6CD5\u6216\u540C\u4E00\u98CE\u683C\u65F6\u5148\u8BFB\u672C\u6280\u80FD\u3002`).slice(0, 400);
  const markdown = skillMarkdown({ name: name2, description, title, body });
  const roots = extraSkillRoots(input.outputDir, input.userRoot);
  const files = roots.map((root) => join37(root, name2, "SKILL.md"));
  if (input.replace !== true && files.some((file) => existsSync11(file))) {
    throw new Error(`\u6280\u80FD ${name2} \u5DF2\u5B58\u5728\u3002\u786E\u8BA4\u8986\u76D6\u5C31\u4F20 replace:true\u3002`);
  }
  const paths = [];
  for (const file of files) {
    await mkdir24(join37(file, ".."), { recursive: true });
    await writeFile20(file, markdown, "utf8");
    paths.push(file);
  }
  skillIndex.invalidate();
  return { name: name2, title, paths, description };
}
...
// src/bible.ts
import { readdir as readdir4, readFile as readFile24, writeFile as writeFile21, mkdir as mkdir25 } from "node:fs/promises";
import { basename as basename4, join as join38 } from "node:path";
var KIND_FILE = {
...
async function walkJson(dir, depth = 0) {
  ...
  const out = [];
  for (const entry of entries) {
    const path = join38(dir, entry.name);
    if (entry.isDirectory()) {
...
function inferBibleKind(path) {
  ...
  for (const kind of Object.keys(KIND_FILE)) {
    if (KIND_FILE[kind].test(name2)) return kind;
  }
  return void 0;
}
async function detectBibles(outputDir) {
  ...
  for (const path of files) {
    ...
    let title = basename4(path).replace(KIND_FILE[kind], "");
    try {
      const parsed = asRecord2(JSON.parse(await readFile24(path, "utf8")));
...
function characterReview(doc, path) {
  ...
  const namedInPrompt = names.filter((name2) => name2.length >= 2 && promptBlob.includes(name2.toLowerCase()));
  const gates = [
  ...
  const source = typeof doc.source === "string" ? doc.source : basename4(path);
  const lines = [
    `# \u8BC4\u5BA1\uFF5C\u89D2\u8272 \xB7 ${source}`,
    "",
...
async function reviewBible(path, kindHint) {
  ...
  if (kind === void 0) throw new Error(`\u65E0\u6CD5\u4ECE\u6587\u4EF6\u540D\u5224\u65AD\u7C7B\u578B\uFF1A${basename4(path)}`);
  const doc = JSON.parse(await readFile24(path, "utf8"));
  ...
  const header = [
    `# \u8BC4\u5BA1\uFF5C${KIND_LABEL2[kind]} \xB7 ${asRecord2(doc).source ?? basename4(path)}`,
    "",
  ...
  const title = typeof asRecord2(doc).source === "string" ? String(asRecord2(doc).source) : basename4(path);
  return packReview(kind, path, title, gates, markdown);
}
async function pinBible(input) {
  const dir = join38(resolveOutputDir(input.outputDir), "docs");
  await mkdir25(dir, { recursive: true });
  const file = join38(dir, `${input.review.kind}-review.md`);
  await writeFile21(file, input.review.markdown, "utf8");
  const canvas = new DirectorxCanvasStore(input.outputDir);
  ...
  const label = input.review.markdown.slice(0, 8e3);
  if (existing !== void 0) {
...
// src/series.ts
import { copyFile, mkdir as mkdir26, readdir as readdir5, readFile as readFile25, writeFile as writeFile22 } from "node:fs/promises";
import { existsSync as existsSync12 } from "node:fs";
import { basename as basename5, extname as extname4, join as join39 } from "node:path";
var ACTIVE = "series-active.json";
var MAX_RULES = 24;
var MAX_NOTES = 16;
function userSeriesRoot(override) {
  return override ?? join39(dshHome(), "series");
}
function projectSeriesRoot(outputDir) {
  return join39(resolveOutputDir(outputDir), "series");
}
...
async function readPackFile(path) {
  try {
    return asPack(JSON.parse(await readFile25(path, "utf8")));
  } catch {
    return void 0;
  }
}
async function listRoot(root, kind) {
  let names = [];
  try {
    names = await readdir5(root);
  } catch {
    return [];
  }
  const items = [];
  for (const name2 of names) {
    const path = join39(root, name2, "pack.json");
...
async function persistPack(pack, root) {
  const dir = join39(root, pack.name);
  const refs = join39(dir, "refs");
  await mkdir26(refs, { recursive: true });
  const characters = [];
  for (const card of pack.characters) {
    const ext = extname4(card.refPath) || ".png";
    const dest = join39(refs, `${card.name.replace(/[^\w\u4e00-\u9fff-]+/g, "").slice(0, 24) || "ref"}${ext}`);
    if (existsSync12(card.refPath) && card.refPath !== dest) {
  ...
  const path = join39(dir, "pack.json");
  await writeFile22(path, JSON.stringify(next, null, 2), "utf8");
  return path;
}
async function saveSeries(input) {
  const harvested = await harvestSeries(input.outputDir, input.title);
  const name2 = typeof input.name === "string" && input.name.trim() !== "" ? slugSeriesName(input.name) : harvested.name;
  const pack = {
    ...harvested,
    name: name2,
  ...
  await writeFile22(join39(resolveOutputDir(input.outputDir), ACTIVE), JSON.stringify({ name: pack.name, at: pack.at }, null, 2), "utf8");
  return { pack, paths };
}
...
async function loadSeries(outputDir, name2, userRoot) {
  const slug = slugSeriesName(name2);
  const candidates = [
    join39(projectSeriesRoot(outputDir), slug, "pack.json"),
    join39(projectSeriesRoot(outputDir), name2, "pack.json"),
    join39(userSeriesRoot(userRoot), slug, "pack.json"),
    join39(userSeriesRoot(userRoot), name2, "pack.json")
  ];
  const listed = await listSeries(outputDir, userRoot);
  const hit = listed.find((item2) => item2.name === slug || item2.name === name2 || item2.title === name2);
  if (hit !== void 0) candidates.unshift(hit.path);
  for (const path of candidates) {
    const pack = await readPackFile(path);
    if (pack !== void 0) return pack;
  }
  throw new Error(`\u627E\u4E0D\u5230\u7CFB\u5217\u5305\u300C${name2}\u300D\u3002\u5148 directorx_series list\u3002`);
}
async function applySeries(input) {
  ...
  const destDir = join39(resolveOutputDir(input.outputDir), "series-refs", pack.name);
  await mkdir26(destDir, { recursive: true });
  const registered = [];
  for (const card of pack.characters) {
    let refPath = card.refPath;
    if (existsSync12(card.refPath)) {
      const dest = join39(destDir, basename5(card.refPath));
  ...
  await writeFile22(join39(resolveOutputDir(input.outputDir), ACTIVE), JSON.stringify({ name: pack.name, at: Date.now() }, null, 2), "utf8");
...
async function activeSeries(outputDir, userRoot) {
  try {
    const raw = JSON.parse(await readFile25(join39(resolveOutputDir(outputDir), ACTIVE), "utf8"));
...
// src/blocking.ts
import { mkdir as mkdir27, writeFile as writeFile23 } from "node:fs/promises";
...
async function harvestBlocking(outputDir) {
  ...
  const seen = /* @__PURE__ */ new Set();
  const unique3 = refs.filter((item2) => {
...
async function pinBlocking(input) {
  ...
  const dir = join40(resolveOutputDir(input.outputDir), "docs");
  await mkdir27(dir, { recursive: true });
  const path = join40(dir, "blocking.md");
  await writeFile23(path, markdown, "utf8");
  const canvas = new DirectorxCanvasStore(input.outputDir);
  const doc = await canvas.read();
...
import { mkdir as mkdir28, readFile as readFile26, writeFile as writeFile24 } from "node:fs/promises";
...
var AdapterStore = class {
  ...
  catalogPath() {
    return join41(resolveOutputDir(this.outputDir), "adapters.json");
  }
  secretsPath() {
    return join41(resolveOutputDir(this.outputDir), "adapters.secrets.json");
  }
  docPath(id) {
    return join41(resolveOutputDir(this.outputDir), "adapters", `${id}.doc.txt`);
  }
  async read() {
    try {
  ...
  async findByModel(capability2, model) {
    ...
    return (await this.read()).adapters.find((item2) => item2.spec.capability === capability2 && item2.spec.model === wanted);
  }
  async readDoc(id) {
    try {
  ...
  async writeSecret(id, secret) {
    ...
    await mkdir28(resolveOutputDir(this.outputDir), { recursive: true });
  ...
  async saveDoc(id, text) {
    ...
    await mkdir28(join41(resolveOutputDir(this.outputDir), "adapters"), { recursive: true });
    ...
    async execute(args) {
      ...
      const results = (await corpus.search(args.query, maxResults, { group, type, tag })).map((hit) => ({
        ...
        next: [
          ...
          ...skillsForArticle(hit.id).slice(0, 2).map((name2) => `directorx_skill_read ${name2}`)
        ]
      }));
