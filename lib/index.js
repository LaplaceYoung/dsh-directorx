...
import { mkdir as mkdir6, readFile as readFile7, readdir, rm, stat } from "node:fs/promises";
...
var DirectorxTaskLedger = class {
  ...
  async filePath() {
    const dir = resolve4(process.cwd(), this.outputDir);
    await mkdir3(dir, { recursive: true });
    return join4(dir, LEDGER_FILE);
  }
  async append(record) {
...
var DirectorxCanvasStore = class {
  ...
  filePath() {
    return join5(resolve5(process.cwd(), this.outputDir), CANVAS_FILE);
  }
  async read() {
    const path = this.filePath();
    const raw = await readFile5(path, "utf8").catch((error) => {
      if (error.code === "ENOENT") return "";
      throw error;
    });
  ...
  async reset() {
    const path = this.filePath();
    try {
      const existing = await readFile5(path, "utf8");
      if (existing.trim() !== "") {
        const backup = join5(resolve5(process.cwd(), this.outputDir), `canvas.json.bak-${Date.now()}`);
        await writeFile2(backup, existing, "utf8");
      }
    } catch {
    }
  ...
  async write(doc, expectedUpdatedAt) {
    const dir = join5(resolve5(process.cwd(), this.outputDir));
    ...
    await writeFile2(path, JSON.stringify(saved), "utf8");
    return saved;
  }
  ...
  snapshotsPath() {
    return join5(resolve5(process.cwd(), this.outputDir), ".canvas-snapshots");
  }
  snapshotsFile() {
    return join5(this.snapshotsPath(), "index.json");
  }
  async snapshot(label) {
    ...
    await mkdir4(this.snapshotsPath(), { recursive: true });
    await writeFile2(this.snapshotsFile(), JSON.stringify(index, null, 2), "utf8");
    await writeFile2(join5(this.snapshotsPath(), `${id}.json`), JSON.stringify(doc, null, 2), "utf8");
    return index[0];
  }
  async readSnapshotsIndex() {
    try {
      const parsed = JSON.parse(await readFile5(this.snapshotsFile(), "utf8"));
  ...
  async restoreSnapshot(id) {
    const raw = await readFile5(join5(this.snapshotsPath(), `${id}.json`), "utf8");
    const parsed = JSON.parse(raw);
...
import { basename, extname, join as join6, resolve as resolve6, sep as sep2 } from "node:path";
var MIME = {
  ".jpg": "image/jpeg",
  ".jpeg": "image/jpeg",
  ".png": "image/png",
...
async function ensureOutputDir(dir) {
  const out = resolve6(process.cwd(), dir);
...
async function saveBase64ToFile(data, outDir, prefix, ext) {
  ...
  const path = join6(outDir, `${prefix}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}${normalizedExt}`);
  await writeFile3(path, Buffer.from(raw, "base64"));
  return path;
}
var MAX_MEDIA_BYTES = 512 * 1024 * 1024;
function resolveMediaPath(outputDir, candidate) {
  const root = resolve6(process.cwd(), outputDir);
  const target = resolve6(root, candidate);
...
async function saveEditedMedia(outputDir, request, response) {
  ...
  const dir = join7(resolve7(process.cwd(), outputDir), EDIT_SUBDIR);
...
function registerVendorRoute(ctx) {
  ...
  const serve = async (name2, contentType, response) => {
    try {
      const dir = vendorDirs.find((candidate) => existsSync2(join7(candidate, name2)));
      if (dir === void 0) throw new Error(`vendor asset ${name2} missing`);
      const data = await readFile7(join7(dir, name2));
      response.writeHead(200, {
...
function registerCanvasResetRoute(ctx, getOutputDir) {
  ...
  return webServer.register({
    ...
    handler: async (request, response) => {
      ...
      try {
        const store = new DirectorxCanvasStore(resolve7(process.cwd(), getOutputDir()));
...
async function readSkillFile(path) {
  try {
    return parseFrontmatter(await readFile8(path, "utf8"));
  } catch {
    return void 0;
  }
}
...
async function registerBundledSkills(ctx) {
  ...
  for (const { dir, safeName } of await firstLevelSkillDirs(root)) {
    const parsed = await readSkillFile(join8(dir, "SKILL.md"));
  ...
  const cnRoot = join8(root, "video-production-cn");
...
async function klingVideo(ctx, prompt, options) {
  ...
  for (let attempt = 0; attempt < settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve18) => setTimeout(resolve18, settings.pollIntervalMs));
...
// src/terms.ts
import { mkdir as mkdir10, readFile as readFile10, writeFile as writeFile5 } from "node:fs/promises";
...
var TermStore = class {
  ...
  filePath() {
    return join13(resolve13(process.cwd(), this.outputDir), "terms.json");
  }
  async read() {
    try {
  ...
  async set(entries) {
    ...
    await mkdir10(resolve13(process.cwd(), this.outputDir), { recursive: true });
    await writeFile5(this.filePath(), JSON.stringify({ terms: ledger }, null, 2), "utf8");
    return ledger;
  }
  /** 按句命中：返回文本中出现的术语及其读法。 */
  async match(text) {
...
async function mockAudio(ctx, text) {
  ...
  const path = join14(outDir, `${slugify(text, 24)}-mock.wav`);
  await writeFile6(path, makeWav());
...
async function openaiTts(ctx, text, voice, format, instructions, speed) {
  ...
  const path = join14(outDir, `${slugify(text, 24)}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}.${ext}`);
...
async function mockImage(ctx, prompt, size) {
  ...
  const path = join15(outDir, name2);
  const svg = [
  ...
  await writeFile7(path, svg, "utf8");
  return {
    model: ctx.capability.model,
    prompt,
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join16(resolve14(process.cwd(), ctx.settings.outputDir), "transcripts");
  await mkdir11(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join16(dir, `${slugify(source, 24)}-${stamp}.srt`);
  await writeFile8(srtPath, srt, "utf8");
  return {
    model: ctx.capability.model,
    source,
    text,
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile11(resolve14(source));
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join16(dir, `${slugify(source, 24)}-${stamp}.txt`);
    await writeFile8(txtPath, text, "utf8");
...
async function minimaxH3Video(ctx, prompt, options) {
  ...
  for (let attempt = 0; attempt < ctx.settings.maxPollAttempts; attempt += 1) {
    ...
    await new Promise((resolve18) => setTimeout(resolve18, ctx.settings.pollIntervalMs));
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join17(outDir, `${slugify(prompt)}-mock.mp4`);
...
// src/characters.ts
import { mkdir as mkdir12, readFile as readFile12, writeFile as writeFile9 } from "node:fs/promises";
...
var CharacterStore = class {
  ...
  filePath() {
    return join18(resolve15(process.cwd(), this.outputDir), "characters.json");
  }
  async read() {
    try {
...
import { copyFileSync, existsSync as existsSync3, readFileSync, statSync, mkdirSync } from "node:fs";
...
function segmentCachePath(outputDir, fingerprint) {
  return join19(resolve16(process.cwd(), outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join19(resolve16(process.cwd(), outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join19(resolve16(process.cwd(), outputDir), `faded-${Date.now().toString(36)}.mp4`);
...
async function videoUnderstand(input) {
  const probe = probeMedia(input.source);
  const framesDir = resolve17(process.cwd(), input.outputDir);
