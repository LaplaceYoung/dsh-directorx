...
var ProductionStageStore = class {
  ...
  filePath() {
    return join28(resolveOutputDir(this.outputDir), "stage.json");
  }
  async read() {
    try {
  ...
  async write(doc) {
    ...
    await writeFile15(this.filePath(), JSON.stringify(doc, null, 2), "utf8");
    return doc;
  }
  async get() {
    const doc = await this.read();
...
var AdapterStore = class {
  ...
  catalogPath() {
    return join29(resolveOutputDir(this.outputDir), "adapters.json");
  }
  secretsPath() {
    return join29(resolveOutputDir(this.outputDir), "adapters.secrets.json");
  }
  docPath(id) {
    return join29(resolveOutputDir(this.outputDir), "adapters", `${id}.doc.txt`);
  }
  async read() {
    try {
  ...
  async saveDoc(id, text) {
    ...
    await mkdir20(join29(resolveOutputDir(this.outputDir), "adapters"), { recursive: true });
...
async function generationGate(settings, store, args) {
...
function syncTools(ctx, settings, applyCapability) {
  const disposers = [];
  const proposals = new ProposalStore(settings.outputDir);
  ...
  if (settings.image.enabled) {
    disposers.push(ctx.tools.register(safeDefine({
      ...
      async execute(args, exec) {
        ...
        if (!gate.generate) {
          return { ...gate, refused: true, next: gate.next ?? (gate.authorized ? "directorx_propose" : "\u5148 directorx_prompt_craft\uFF0C\u4E25\u683C/\u534F\u540C\u518D directorx_confirm") };
        }
        const signal = combinedSignal(exec.signal, settings.timeoutMs);
  ...
  if (settings.video.enabled) {
    disposers.push(ctx.tools.register(safeDefine({
      ...
      async execute(args, exec) {
        const gate = await generationGate(settings, proposals, args);
        if (!gate.generate) {
          return { ...gate, refused: true, next: gate.next ?? "\u5148 directorx_prompt_craft\uFF0C\u518D confirm/generate" };
        }
