...
var VISION_MODES = ["deepseek-chat", "openai-chat", "mock"];
...
function audioMixGraph(tracks, duckUnder, targetLufs) {
  const parts = [];
  ...
  if (duckUnder !== void 0 && duckUnder >= 0 && duckUnder < tracks.length) {
...
// src/modelverse-catalog.ts
var DEEPSEEK_PUBLIC_BASE_URL = "https://api.deepseek.com";

// src/providers/vision.ts
var DEEPSEEK_IMAGE_MIMES = /* @__PURE__ */ new Set(["image/png", "image/jpeg", "image/webp", "image/gif"]);
var DEEPSEEK_MAX_IMAGE_BYTES = 15 * 1024 * 1024;
...
function assertDeepSeekImage(dataUrl) {
...
async function inlineImageForDeepSeek(source) {
...
async function postChatCompletion(baseURL, apiKey, model, content, signal) {
...
async function deepseekVision(ctx, source, question) {
...
// src/prompt-craft.ts
import { mkdir as mkdir15, readFile as readFile16, writeFile as writeFile10 } from "node:fs/promises";
import { join as join24 } from "node:path";

// src/skill-index.ts
import { readdir as readdir3, readFile as readFile14 } from "node:fs/promises";
import { join as join22, relative as relative2, resolve as resolve9, sep as sep3 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
var MAX_READ = 4e4;
var expandQuery2 = expandCraftQuery;
var tokens = textTokens;
var overlap2 = overlapScore;
var SkillIndex = class {
  root = resolve9(process.cwd(), "skills");
  extraRoots = [];
  cache;
  setRoot(root) {
  ...
  setExtraRoots(roots) {
    this.extraRoots = [...new Set(roots.map((item2) => resolve9(item2)))];
    this.cache = void 0;
  }
  invalidate() {
    this.cache = void 0;
  ...
  async scan() {
    ...
    const walkRoot = async (root) => {
      const walk = async (dir) => {
        ...
        for (const entry of entries) {
          ...
          const body = await readFile14(path, "utf8").catch(() => "");
          if (body === "") continue;
          ...
          for (const child of sibling) {
            if (child.isDirectory() && (child.name === "references" || child.name === "examples")) {
              const nested = await readdir3(join22(dir, child.name)).catch(() => []);
    ...
    for (const extra of this.extraRoots) {
      if (resolve9(extra) === this.root) continue;
  ...
  async read(name2, file) {
    ...
    if (file !== void 0 && file.trim() !== "") {
      ...
      const path = join22(record.absDir, rel);
...
import { mkdir as mkdir14, readFile as readFile15, writeFile as writeFile9 } from "node:fs/promises";
...
var ResearchLedger = class {
  ...
  filePath() {
    return join23(resolveOutputDir(this.outputDir), FILE4);
  }
  async read() {
    try {
  ...
  async record(event) {
    ...
    await writeFile9(this.filePath(), JSON.stringify({ events: next }, null, 2), "utf8");
  }
...
var PromptCraftStore = class {
  ...
  filePath() {
    return join24(resolveOutputDir(this.outputDir), FILE5);
  }
  async read() {
    try {
      const parsed = JSON.parse(await readFile16(this.filePath(), "utf8"));
  ...
  async save(craft) {
    ...
    await writeFile10(this.filePath(), JSON.stringify({ crafts: crafts.slice(-MAX4) }, null, 2), "utf8");
    return craft;
  }
};
...
import { mkdir as mkdir16, readFile as readFile17, writeFile as writeFile11 } from "node:fs/promises";
...
var ProjectStyleStore = class {
  ...
  filePath() {
    return join25(resolveOutputDir(this.outputDir), "style.json");
  }
  async read() {
    try {
  ...
  async set(input) {
    ...
    await mkdir16(resolveOutputDir(this.outputDir), { recursive: true });
    await writeFile11(this.filePath(), JSON.stringify(merged, null, 2), "utf8");
    return merged;
  }
  /** 生成提示词的常量块（逐字复用）。 */
  block() {
...
// src/terms.ts
import { mkdir as mkdir17, readFile as readFile18, writeFile as writeFile12 } from "node:fs/promises";
...
var TermStore = class {
  ...
  filePath() {
    return join26(resolveOutputDir(this.outputDir), "terms.json");
  }
  async read() {
    try {
  ...
  async set(entries) {
    ...
    await writeFile12(this.filePath(), JSON.stringify({ terms: ledger }, null, 2), "utf8");
    return ledger;
  }
  /** 按句命中：返回文本中出现的术语及其读法。 */
  async match(text) {
...
async function orchestrateProduction(input) {
  ...
  const run = {
    ...
    reportPath: join27(resolveOutputDir(input.outputDir), `orchestrate-${kind}.json`)
  };
  await mkdir18(resolveOutputDir(input.outputDir), { recursive: true });
  await writeFile13(run.reportPath, JSON.stringify(run, null, 2), "utf8");
  return run;
}
...
async function mockAudio(ctx, text) {
  ...
  const path = join28(outDir, `${slugify(text, 24)}-mock.wav`);
  await writeFile14(path, makeWav());
...
async function openaiTts(ctx, text, voice, format, instructions, speed) {
  ...
  const path = join28(outDir, `${slugify(text, 24)}-${(/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z")}.${ext}`);
  await writeFile14(path, bytes);
...
import { readFile as readFile19, writeFile as writeFile15 } from "node:fs/promises";
import { basename as basename3, join as join29, resolve as resolve11 } from "node:path";
...
async function mockImage(ctx, prompt, size) {
  ...
  const path = join29(outDir, name2);
  const svg = [
  ...
  await writeFile15(path, svg, "utf8");
  return {
    model: ctx.capability.model,
    prompt,
...
function resolveLocalImage(source) {
  ...
  const project = currentProjectRoot();
  const hit = [resolve11(source), resolve11(project, source)].find((candidate) => existsSync8(candidate));
...
async function openaiImageEdit(ctx, prompt, size, quality, referenceImagePaths) {
  ...
  for (const source of referenceImagePaths.slice(0, 4)) {
    ...
    const bytes = await readFile19(path);
...
import { extname as extname3, join as join30 } from "node:path";
...
async function imageProcess(input) {
  ...
  const out = join30(resolveOutputDir(input.outputDir), `${slugify("image-edit")}-${Date.now().toString(36)}.${ext}`);
...
import { mkdir as mkdir19, readFile as readFile20, writeFile as writeFile16 } from "node:fs/promises";
import { join as join31, resolve as resolve12 } from "node:path";
...
async function mockTranscribe(ctx, source) {
  ...
  const dir = join31(resolveOutputDir(ctx.settings.outputDir), "transcripts");
  await mkdir19(dir, { recursive: true });
  const stamp = (/* @__PURE__ */ new Date()).toISOString().replaceAll(":", "-").replace(/\.\d+Z$/, "Z");
  const srtPath = join31(dir, `${slugify(source, 24)}-${stamp}.srt`);
  await writeFile16(srtPath, srt, "utf8");
  return {
    model: ctx.capability.model,
    source,
    text,
...
async function openaiTranscribe(ctx, source, options) {
  const baseURL = ctx.capability.baseURL.replace(/\/+$/, "");
  const apiKey = apiKeyOf(ctx.capability.apiKey, ["DIRECTORX_AUDIO_API_KEY", "OPENAI_API_KEY"], baseURL);
  const bytes = await readFile20(resolve12(source));
  const form = new FormData();
  ...
  if (srt !== void 0) {
    ...
    const txtPath = join31(dir, `${slugify(source, 24)}-${stamp}.txt`);
    await writeFile16(txtPath, text, "utf8");
...
async function mockVideo(ctx, prompt) {
  const outDir = await ensureOutputDir(ctx.settings.outputDir);
  const path = join32(outDir, `${slugify(prompt)}-mock.mp4`);
...
function segmentCachePath(outputDir, fingerprint) {
  return join33(resolveOutputDir(outputDir), ".timeline-cache", `${fingerprint}.mp4`);
}
  ...
  try {
    ...
    for (const [index, scene] of spec.scenes.entries()) {
      ...
      if (scene.trim !== void 0) {
        ...
        try {
          mkdirSync(join33(resolveOutputDir(outputDir), ".timeline-cache"), { recursive: true });
    ...
    if (spec.fadeIn !== void 0 || spec.fadeOut !== void 0) {
      ...
      if (fadeFilters.length > 0 || audioFade.length > 0) {
        const out = join33(resolveOutputDir(outputDir), `faded-${Date.now().toString(36)}.mp4`);
...
async function videoUnderstand(input) {
  ...
  await mkdir20(framesDir, { recursive: true });
...
function resolveHostAsk(ctx) {
  for (const name2 of ["userQuestions", "userInteraction"]) {
    try {
      const service = ctx.get(name2);
      if (service !== void 0 && typeof service.ask === "function") return service;
    } catch {
    }
  }
  return void 0;
...
async function runSkillCapture(input) {
  ...
  if (action === "offer") {
    ...
    if (input.present === true) {
      if (input.ask === void 0) throw new Error("directorx_skill_capture present \u9700\u8981 DSH userQuestions");
...
async function pinBible(input) {
  ...
  if (input.review.kind === "characters" || input.review.kind === "script") {
    try {
      const raw = asRecord2(JSON.parse(await readFile24(input.review.path, "utf8")));
...
${blocks.join("\uFF1B")}` : gate.prompt;
  ...
  disposers.push(ctx.tools.register(safeDefine({
    ...
    async execute(args, exec) {
      ...
      if (args.present === true && hostAsk === void 0) {
      ...
      const result = await runSkillCapture({
        ...
        ...args.present === true && hostAsk !== void 0 ? { ask: (request) => hostAsk.ask(request), agent: exec.agent, signal: exec.signal } : {}
      });
  ...
  disposers.push(ctx.tools.register(safeDefine({
    ...
    async execute(args, exec) {
      ...
      if (args.present === true && Array.isArray(ask) && ask.length > 0) {
        const hostAsk = resolveHostAsk(ctx);
        if (hostAsk === void 0) throw new Error("directorx_generate_ready present \u9700\u8981 DSH userQuestions");
        answers = (await presentAsk({
  ...
  disposers.push(ctx.tools.register(safeDefine({
    ...
    async execute(args, exec) {
      ...
      if (args.present === true && ask.length > 0) {
        const hostAsk = resolveHostAsk(ctx);
        if (hostAsk === void 0) throw new Error("directorx_chengpian present \u9700\u8981 DSH userQuestions");
        answers = (await presentAsk({
          questions: normalizeAskQuestions(ask),
          ask: (request) => hostAsk.ask(request),
          agent: exec.agent,
