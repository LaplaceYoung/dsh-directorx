...
function resolveAssetUrl(path: string): string {
  if (!path) return path;
  if (/^https?:/i.test(path) && !/^https?:\/\/(127\.0\.0\.1|localhost|[\w.-]+\.my\.local-ip\.co)(:\d+)?\b/i.test(path)) return '';
...
export class DirectorEngine {
  ...
  private monitorContainer: HTMLDivElement | null = null;
  private orbit: OrbitControls;
  private gizmo: TransformControls;
  private gizmoHelper: THREE.Object3D;
  private transformControlsVisible = true;
  private cameraHelpersVisible = false;
  ...
  constructor(container: HTMLDivElement) {
    ...
    const loop = () => {
      ...
      if (this.monitorRenderer) {
        ...
        this.setShotHelpersVisible(false, true);
  ...
  resize(): void {
    ...
    this.renderer.setSize(w, h);
    if (this.monitorRenderer && this.monitorContainer) {
      const monitorWidth = this.monitorContainer.clientWidth;
      const monitorHeight = this.monitorContainer.clientHeight;
      if (monitorWidth && monitorHeight) this.monitorRenderer.setSize(monitorWidth, monitorHeight, false);
    }
  }
  ...
  syncElements(elements: DirectorElement[]): void {
    ...
    for (const el of elements) {
      ...
      const needRebuild = !obj
        || (prev && (
          prev.kind !== el.kind
          || (el.kind === 'mannequin' && ((prev as MannequinElement).heightM !== el.heightM || (prev as MannequinElement).bodyType !== el.bodyType || (prev as MannequinElement).figure !== el.figure || (prev as MannequinElement).animalSpecies !== el.animalSpecies))
  ...
  private buildObject(el: DirectorElement): THREE.Object3D {
    switch (el.kind) {
      ...
      case 'mannequin': {
        return el.bodyType === 'animal' ? buildAnimal(el.heightM, el.color, el.animalSpecies) : buildMannequin(el.heightM, el.color, el.figure ?? 'male');
      }
      case 'billboard': {
        const group = new THREE.Group();
  ...
  setCameraHelpersVisible(visible: boolean): void {
    this.cameraHelpersVisible = visible;
  ...
  private setShotHelpersVisible(visible: boolean, actorLabelsVisible = visible): void {
    this.shotCameraBody.visible = visible && this.cameraHelpersVisible;
    this.shotCameraHelper.visible = visible && this.cameraHelpersVisible;
    this.shotTargetMarker.visible = visible && this.cameraHelpersVisible;
    this.shotPath.visible = visible && this.cameraHelpersVisible;
    this.actionPathGroup.visible = visible;
    for (const object of this.objects.values()) object.traverse((child) => { if (child.userData.isDirectorActorLabel) child.visible = actorLabelsVisible; });
  }
  ...
  captureShot(aspect: AspectId, width = 1600, includeActorLabels = false): string {
    return this.captureFrame(aspect, width, { includeHelpers: false, includeActorLabels, transparent: false });
  }

  captureTransparent(aspect: AspectId, width = 1600): string {
    return this.captureFrame(aspect, width, { includeHelpers: false, includeActorLabels: false, transparent: true });
  }
  ...
  captureTopView(aspect: AspectId, width = 1600, includePaths = false): string {
    ...
    this.shotCamera.updateProjectionMatrix();
    try {
      return this.captureFrame(aspect, width, { includeHelpers: includePaths, includeActorLabels: false, transparent: false });
    } finally {
  ...
  private captureFrame(aspect: AspectId, width: number, options: { includeHelpers: boolean; includeActorLabels: boolean; transparent: boolean }): string {
    ...
    try {
      ...
      this.gizmoHelper.visible = false;
      this.setShotHelpersVisible(options.includeHelpers, options.includeActorLabels);
  ...
  setPanorama(imagePath: string | null): void {
    ...
    this.texLoader.load(resolveAssetUrl(imagePath), (tex) => {
