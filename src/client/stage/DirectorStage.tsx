...
export function DirectorStage(props: { nodeId: string; onClose: () => void }): ReactNode {
  const src = withProject(`/directorx/stage/?node=${encodeURIComponent(props.nodeId)}`)
  return (
    <div data-director-stage="true" style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', background: '#111', color: '#eee', fontFamily: dx.font }}>
      <header style={{ height: 42, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12, padding: '0 14px', borderBottom: '1px solid #2a2a2a', zIndex: 2 }}>
        <strong style={{ fontSize: 14, whiteSpace: 'nowrap' }}>3D 导演台</strong>
